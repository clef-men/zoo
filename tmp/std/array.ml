...
let mapi t fn =
  unsafe_initi (size t) (fun i -> fn i (unsafe_get i))
let map t fn =
  mapi t (fun _ -> fn)

let unsafe_copy_slice t1 i1 t2 i2 n =
  for i = 0 to n - 1 do
    let v = unsafe_get t1 (i1 + n) in
    unsafe_set t2 (i2 + n) v
  done
let copy_slice t1 i2 t2 i2 n =
  let sz1 = size t1 in
  let sz2 = size t2 in
  if not @@ 0 <= i1 then
    invalid_arg "negative index" ;
...
let unsafe_ccopy_slice t1 i1 t2 i2 n =
  ...
  let sz2 = size t2 in
  for j = 0 to n - 1 do
    let v = unsafe_get t1 ((i1 + j) mod sz1) in
    unsafe_set t2 ((i2 + j) mod sz2) v
  done
let ccopy_slice t1 i1 t2 i2 n =
  ...
  if not @@ 0 <= n then
    invalid_arg "negative span" ;
  if not @@ n <= size t1 then
    invalid_arg "invalid span" ;
  if not @@ n <= size t2 then
    invalid_arg "invalid span" ;
