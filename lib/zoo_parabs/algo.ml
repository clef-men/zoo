...
let rec fold ctx ~beg ~end_ ~chunk ~body ~op ~zero =
  let num_task = end_ - beg in
  if num_task <= chunk then
    fold_seq ctx ~beg ~end_:(beg + num_task) ~body ~op zero
  else
    let mid = beg + num_task / 2 in
    let left =
      Future.async ctx @@ fun ctx ->
        fold ctx ~beg ~end_:mid ~chunk ~body ~op ~zero
    in
    let right = fold ctx ~beg:mid ~end_ ~chunk ~body ~op ~zero in
...
let rec find ctx ~beg ~end_ ~chunk pred found =
  let num_task = end_ - beg in
  if num_task <= chunk then
    find_seq ctx ~beg ~end_:(beg + num_task) pred found
  else if Mvar.is_unset found then
    let mid = beg + num_task / 2 in
    let left =
      Future.async ctx @@ fun ctx ->
        find ctx ~beg ~end_:mid ~chunk pred found
    in
    find ctx ~beg:mid ~end_ ~chunk pred found ;
    Future.wait ctx left
