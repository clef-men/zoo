...
Section mpsc_queue_1_G.
  ...
  Proof.
    ...
    iDestruct (mpsc_queue_1_history_elem_get (length past) front with "Hhistory_auth") as "#Hhistory_elem".
  ...
  Proof.
    ...
    wp_load.
    iDestruct (mpsc_queue_1_history_elem_get _ front with "Hhistory_auth") as "#Hhistory_elem".
    { rewrite Hhist list_lookup_middle //. }
  ...
  Proof.
    ...
    iMod (mpsc_queue_1_inv_inner_history_elem with "Hinv Hl_front") as "(%i & Hl_front & #Hhistory_elem)".
  ...
  Proof.
    ...
    wp_rec.
    wp_smart_apply (mpsc_queue_1_xchain_next_spec with "[$Hmeta $Hinv $Hhistory_elem]") as (res) "[-> | (%node' & -> & #Hhistory_elem')]"; last iSteps.
    wp_pures.
  ...
  Proof.
    ...
    { wp_smart_apply (mpsc_queue_1_xchain_next_spec with "[$Hmeta $Hinv $Hhistory_elem_new_back]") as (res) "[-> | (%new_back' & -> & #Hhistory_elem_new_back')]"; last iSteps.
      wp_pures.

      wp_bind (CAS _ _ _).
      iInv "Hinv" as "(%hist & %past & %front & %nodes & %back' & %vs & >%Hhist & >%Hback & Hl_front & Hl_back & Hhist & Hnodes & Hhistory_auth & Hmodel₂)".
      wp_cas as _ | [= ->].
      2: iDestruct (mpsc_queue_1_history_agree with "Hhistory_auth Hhistory_elem_new_back") as %Hnew_back%elem_of_list_lookup_2.
      all: iSplitL; first (repeat iExists _; iSteps).
      all: iSteps.
    }
    destruct b; last iSteps.
    wp_smart_apply (mpsc_queue_1_back_spec with "Hinv") as (back' i') "#Hhistory_elem_back'".
  ...
  Proof.
    ...
    wp_smart_apply (mpsc_queue_1_back_spec with "Hinv") as (back i) "#Hhistory_elem_back".
    wp_smart_apply (mpsc_queue_1_do_push_spec with "[$Hmeta $Hinv $Hhistory_elem_back $Hnew_back_next $Hnew_back_data]").
  ...
  Proof.
    ...
    iMod (mpsc_queue_1_inv_inner_history_elem with "Hinv Hl_front_1") as "(%i & Hl_front_1 & #Hhistory_elem)".

    wp_rec. wp_load.
    wp_smart_apply (mpsc_queue_1_xchain_next_spec_strong MpscQueue1Pop _ _ [tele_arg] inhabitant with "[$Hmeta $Hinv $Hhistory_elem $Hl_front_1 $HΦ]") as (res) "[(-> & Hl_front_1 & HΦ) | (%front' & -> & #Hhistory_elem' & Hl_front_1 & HΦ)]"; [auto | iSteps |].
    wp_pures.
