...
Section zoo_G.
  ...
  Proof.
    ...
    rewrite -{1}Hnodes xchain_model_app // (xchain_model_cons (node :: _)) //.
  ...
  Qed.
  Lemma xchain_model_lookup'_1 {nodes} i node dst :
    nodes !! i = Some node →
    xchain_model nodes dst ⊢
      xchain_model (take i nodes) #node ∗
      node.[xchain_next] ↦ from_option #@{location} dst (nodes !! S i) ∗
  ...
  Qed.
  Lemma xchain_model_lookup'_2 {nodes} i node next dst :
    nodes !! i = Some node →
