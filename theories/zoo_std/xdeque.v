...
Section zoo۰G.
  ...
  Lemma xdeque۰modelｰexclusive t nodes1 nodes2 :
  ...
  Lemma xdeque۰modelｰNoDup t nodes :
    xdeque۰model t nodes ⊢
    ⌜NoDup nodes⌝.
  Proof.
    iIntros "(%l & -> & _ & _ & Hnodes)".
    iApply (xdlchainｰNoDup with "Hnodes").
  Qed.

  Lemma xdeque٠createｰspec :
    {{{
      True
    }}}
      xdeque٠create ()
  ...
  Lemma xdeque٠is_emptyｰspec t nodes :
    {{{
      xdeque۰model t nodes
    }}}
      xdeque٠is_empty t
  ...
  Proof.
    ...
    - case_bool_decide; last iSteps.
      subst.
      iDestruct (xdlchainｰcons₁ with "Hnodes") as "(Hnode_prev & _)"; first done.
      iDestruct (pointstoｰexclusive with "Hprev Hnode_prev") as %[].
  Qed.
  ...
  Proof.
    ...
    wp۰apply+ (xdeque٠linkｰspec with "[$Hnext $Hnode_prev]") as "(Hnext & Hnode_prev)".
    wp۰pures.
    destruct nodes as [| node' nodes] => /=.
    - wp۰apply (xdeque٠linkｰspec with "[$Hnode_next $Hprev]") as "(Hnode_next & Hprev)".
      iSteps.
      iApply (xdlchainｰcons₂ _ _ [] with "Hnode_prev Hnode_next").
      iApply xdlchainｰnil.
    - iDestruct (xdlchainｰcons₁ with "Hnodes") as "(Hnode'_prev & Hnode'_next & Hnodes)"; first done.
      wp۰apply (xdeque٠linkｰspec with "[$Hnode_next $Hnode'_prev]") as "(Hnode_next & Hnode'_prev)".
      iSteps.
      iApply (xdlchainｰcons₂ _ _ (node' :: nodes) with "Hnode_prev Hnode_next").
      iApply (xdlchainｰcons₂ with "Hnode'_prev Hnode'_next Hnodes").
  Qed.
  ...
  Proof.
    ...
    - wp۰apply (xdeque٠linkｰspec with "[$Hnext $Hnode_prev]") as "(Hnext & Hnode_prev)".
      ...
      iApply xdlchainｰnil.
    - rewrite last_snoc /=.
      iDestruct (xdlchainｰsnoc₁ with "Hnodes") as "(Hnodes & Hnode'_prev & Hnode'_next)"; first done.
      wp۰apply (xdeque٠linkｰspec with "[$Hnode'_next $Hnode_prev]") as "(Hnode'_next & Hnode_prev)".
      wp۰apply+ (xdeque٠linkｰspec with "[$Hnode_next $Hprev]") as "(Hnode_next & Hprev)".
      iSteps; [iPureIntro.. |].
      + rewrite last_snoc //.
      + rewrite -assoc head_snoc_snoc //.
      + iApply (xdlchainｰsnoc₂ _ (nodes ++ [node']) with "[Hnodes Hnode'_prev Hnode'_next] [Hnode_prev] Hnode_next"); last rewrite last_snoc //.
        iApply (xdlchainｰsnoc₂ with "Hnodes Hnode'_prev Hnode'_next").
  Qed.

  Lemma xdeque٠pop_frontｰspec t nodes :
    {{{
      xdeque۰model t nodes
    }}}
      xdeque٠pop_front t
  ...
  Proof.
    iIntros "%Φ Hmodel HΦ".
    wp۰rec.
    wp۰apply (xdeque٠is_emptyｰspec with "Hmodel") as "(%l & -> & Hprev & Hnext & Hnodes)".
    case_bool_decide.
    - subst. iSteps.
    - wp۰load.
      destruct nodes as [| node nodes] => //=.
      iDestruct (xdlchainｰcons₁ with "Hnodes") as "(Hnode_prev & Hnode_next & Hnodes)"; first done.
      wp۰load. wp۰pures.
      destruct nodes as [| node' nodes] => /=.
      + wp۰apply (xdeque٠linkｰspec with "[$Hnext $Hprev]") as "(Hnext & Hprev)".
        iSteps.
      + iDestruct (xdlchainｰcons₁ with "Hnodes") as "(Hnode'_prev & Hnode'_next & Hnodes)"; first done.
      wp۰apply (xdeque٠linkｰspec with "[$Hnext $Hnode'_prev]") as "(Hnext & Hnode'_prev)".
      iSteps.
      iApply (xdlchainｰcons₂ with "Hnode'_prev Hnode'_next Hnodes").
  Qed.

  Lemma xdeque٠pop_backｰspec t nodes :
    {{{
      xdeque۰model t nodes
    }}}
      xdeque٠pop_back t
  ...
  Proof.
    ...
    - wp۰load.
      ...
      rewrite last_snoc /=.
      iDestruct (xdlchainｰsnoc₁ with "Hnodes") as "(Hnodes & Hnode1_prev & Hnode1_next)"; first done.
      wp۰load.
      ...
      + wp۰apply+ (xdeque٠linkｰspec with "[$Hnext $Hprev]") as "(Hnext & Hprev)".
        ...
        iExists []. iSteps.
      + rewrite last_snoc.
        iDestruct (xdlchainｰsnoc₁ with "Hnodes") as "(Hnodes & Hnode2_prev & Hnode2_next)"; first done.
        wp۰apply+ (xdeque٠linkｰspec with "[$Hnode2_next $Hprev]") as "(Hnode2_next & Hprev)".
        wp۰pures.
        iApply ("HΦ" $! (Some _)).
        iSteps; first iPureIntro.
        * rewrite last_snoc //.
        * rewrite -assoc head_snoc_snoc //.
        * iApply (xdlchainｰsnoc₂ with "Hnodes Hnode2_prev Hnode2_next").
  Qed.

  Lemma xdeque٠removeｰspec {t nodes} i node :
    nodes !! i = Some node →
    {{{
      xdeque۰model t nodes
    }}}
  ...
  Proof.
    ...
    wp۰rec.
    wp۰apply (xdlchain٠prevｰspecｰlookup with "Hnodes") as "Hnodes"; first done.
    wp۰apply+ (xdlchain٠nextｰspecｰlookup with "Hnodes") as "Hnodes"; first done.
    wp۰pures. wp۰rec. wp۰pures.
    iDestruct (xdlchainｰlookup with "Hnodes") as "(Hnodes1 & Hnode_prev & Hnode_next & Hnodes2)"; first done.
    ...
    { destruct nodes1 as [| node1 nodes1' _] eqn:Hnodes1 using rev_ind => /=; first iSteps.
      rewrite last_snoc /=.
      iDestruct (xdlchainｰsnoc₁ with "Hnodes1") as "(Hnodes1 & Hnode1_prev & Hnode1_next)"; first done.
      wp۰store.
      iDestruct (xdlchainｰsnoc₂ with "Hnodes1 Hnode1_prev Hnode1_next") as "Hnodes1".
      iSteps. iPureIntro.
      rewrite -(take_drop i nodes) -/nodes1 /nodes' Hnodes1 -!assoc !headｰappｰcons //.
    }
    ...
    { destruct nodes2 as [| node2 nodes2'] eqn:Hnodes2 => /=.
      ...
      - iDestruct (xdlchainｰcons₁ with "Hnodes2") as "(Hnode2_prev & Hnode2_next & Hnodes2)"; first done.
        wp۰store.
        iDestruct (xdlchainｰcons₂ with "Hnode2_prev Hnode2_next Hnodes2") as "Hnodes2".
        iSteps. iPureIntro.
        rewrite -(take_drop ˖i nodes) -/nodes2 /nodes' Hnodes2 !last_app_cons //.
    }

    iDestruct (xdlchainｰapp₂ with "Hnodes1 Hnodes2") as "Hnodes".
    rewrite /nodes' -delete_take_drop. iSteps.
  Qed.

  #[local] Lemma xdeque٠iter_auxｰspec Ψ i fn l nodes node :
    (nodes ++ [l]) !! i = Some node →
    {{{
      ▷ Ψ (take i nodes) ∗
  ...
  Proof.
    ...
    - rewrite lookup_app_l in Hlookup; first lia.
      iDestruct (xdlchainｰlookupｰacc with "Hnodes") as "(Hnode_prev & Hnode_next & Hnodes)"; first done.
      iAssert ⌜node ≠ l⌝%I as %Hnode.
      { iIntros "->".
        iApply (pointstoｰexclusive with "Hnode_prev Hprev").
      }
      rewrite bool_decide_eq_false_2 //.
      wp۰apply+ (wpｰwand with "(Hfn [%] HΨ)") as (res) "(-> & HΨ)".
      { erewrite take_drop_middle => //. }
      wp۰load.
      iEval (rewrite from_optionｰdefault).
      ...
      { rewrite headｰdrop.
        ...
        - apply lengthｰlookupｰlast in Hlookup'; last done.
  ...
  Qed.
  Lemma xdeque٠iterｰspec Ψ fn t nodes :
    {{{
      ▷ Ψ [] ∗
      xdeque۰model t nodes ∗
      □ (
  ...
  Proof.
    ...
    wp۰rec. wp۰load.
    iEval (rewrite from_optionｰdefault).
    wp۰apply (xdeque٠iter_auxｰspec Ψ 0 with "[-HΦ] HΦ").
    { destruct nodes; done. }
    iSteps.
  Qed.
End zoo۰G.
