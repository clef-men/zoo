...
Section mutex۰G.
  ...
  Proof.
    iSteps.
  Qed.
  Lemma condition٠createｰdiaspec :
    DIASPEC
    {{
      True
    }}
  ...
  Proof.
    iSteps.
  Qed.
  Lemma condition٠notifyｰdiaspec t :
    DIASPEC
    {{
      condition۰inv t
    }}
  ...
  Lemma condition٠notify_allｰspec t :
    {{{
      condition۰inv t
    }}}
  ...
  Proof.
    iSteps.
  Qed.
  #[global] Instance condition٠notify_allｰdiaspec t :
    DIASPEC
    {{
      condition۰inv t
    }}
  ...
  Lemma condition٠waitｰspec t mtx P :
    {{{
      condition۰inv t ∗
      mutex۰inv mtx P ∗
      mutex۰locked mtx ∗
  ...
  Proof.
    iSteps.
  Qed.
  Lemma condition٠waitｰdiaspec t mtx P :
    DIASPEC
    {{
      condition۰inv t ∗
      mutex۰inv mtx P ∗
  ...
  Lemma condition٠wait_untilｰspec' Ψ t mtx pred P :
    {{{
      condition۰inv t ∗
      mutex۰inv mtx P ∗
      mutex۰locked mtx ∗
  ...
  Proof.
    ...
    wp۰apply (wpｰwand with "(Hpred Hmutex_locked HP HΨ)") as (res) "(%b & -> & Hmutex_locked & HΨ)".
    destruct b; first iSteps.
    iDestruct "HΨ" as "(HP & HΨ)".
    wp۰apply+ (condition٠waitｰspec _ _ P with "[$]") as "(Hmutex_locked & HP)".
  ...
  Lemma condition٠wait_whileｰspec' Ψ t mtx pred P :
    {{{
      condition۰inv t ∗
      mutex۰inv mtx P ∗
      mutex۰locked mtx ∗
  ...
  Proof.
    ...
    wp۰rec.
    wp۰apply+ (condition٠wait_untilｰspec' (λ b, Ψ (￢ b)) _ _ _ P with "[$Hmutex_locked $HP $HΨ]"); last iSteps.
    iSteps. case_match; iSteps.
  Qed.
  Lemma condition٠wait_whileｰspec Ψ t mtx pred P :
    {{{
  ...
  Proof.
    ...
    wp۰apply (condition٠wait_whileｰspec' (λ b,
      (if b then True else P) ∗
      Ψ b
    )%I with "[$Hinv $Hmutex_inv $Hmutex_locked $HP $HΨ] HΦ").
    iSteps. case_match; iSteps.
