...
Section zoo۰G.
  ...
  #[local] Lemma for𑁒spec𑁒stronger beg i δ Ψ _end body :
    i = (beg + δ)%Z →
    {{{
      ▷ Ψ i δ ∗
      □ (
  ...
  Proof.
    ...
    - rewrite decide_False; first lia.
      wp۰apply (wp𑁒wand with "(Hbody [//] [%] HΨ)") as "%res (-> & HΨ)"; first lia.
  ...
  Qed.
  Lemma for𑁒spec𑁒strong Ψ beg _end body :
    {{{
      ▷ Ψ beg 0 ∗
      □ (
        ∀ i δ,
  ...
  Proof.
    ...
    wp۰apply (for𑁒spec𑁒stronger with "[$HΨ $Hbody]"); first lia.
    iSteps.
  Qed.
  ...
  Proof.
    iIntros "% %Φ H HΦ".
    wp۰apply (for𑁒spec𑁒strong Ψ with "H").
    rewrite Z.max_r //.
  Qed.
  Lemma for𑁒spec𑁒strong' Ψ beg _end body :
    {{{
  ...
  Proof.
    ...
    wp۰apply (for𑁒spec𑁒strong Ψ' with "[HΨ Hbody]"); last iSteps.
  ...
  Proof.
    ...
    rewrite Z.max_r //.
  Qed.
  Lemma for𑁒spec𑁒disentangled Ψ beg _end body :
    {{{
      □ (
        ∀ i δ,
        ⌜i = (beg + δ)%Z⌝ -∗
  ...
  Proof.
    ...
    wp۰apply (for𑁒spec𑁒strong Ψ'); last iSteps. iSplit; first iSteps. iIntros "!> %i %δ %Hi1 %Hi2 HΨ'".
    wp۰apply (wp𑁒wand with "(Hbody [//] [//])") as "%res (-> & HΨ)".
    iStep.
  ...
  Proof.
    ...
    wp۰apply (for𑁒spec𑁒strong' Ψ' with "[Hbody]"); last iSteps. iSplit; first iSteps.
    iApply (big_sepL𑁒seq𑁒impl with "Hbody"). iIntros "!> %δ %Hδ Hbody %i -> HΨ' /=".
    wp۰apply (wp𑁒wand with "(Hbody [//])") as "%res (-> & HΨ)".
    iStep.
    rewrite /Ψ' seq_S big_sepL_snoc. iSteps.
  Qed.

  Lemma for𑁒spec𑁒nat𑁒strong Ψ beg _end body :
    {{{
      ▷ Ψ beg 0 ∗
      □ (
        ∀ i δ,
  ...
  Qed.
  Lemma for𑁒spec𑁒nat Ψ beg _end body :
    beg ≤ _end →
    {{{
      ▷ Ψ beg 0 ∗
      □ (
  ...
  Proof.
    iIntros "% %Φ H HΦ".
    wp۰apply (for𑁒spec𑁒nat𑁒strong Ψ with "H").
    rewrite Nat.max_r //.
  Qed.
  Lemma for𑁒spec𑁒nat𑁒strong' Ψ beg _end body :
    {{{
  ...
  Proof.
    ...
    pose Ψ' i δ :=
      Ψ ₊i δ.
    wp۰apply (for𑁒spec𑁒strong' Ψ' with "[HΨ Hbody]").
  ...
  Proof.
    ...
    rewrite Nat.max_r //.
  Qed.
  Lemma for𑁒spec𑁒disentangled𑁒nat Ψ beg _end body :
    {{{
      □ (
        ∀ i δ,
        ⌜i = (beg + δ)%nat⌝ -∗
  ...
  Proof.
    ...
    pose Ψ' i δ :=
      Ψ ₊i δ.
    wp۰apply (for𑁒spec𑁒disentangled Ψ').
    - iIntros "!> %i %δ -> %Hδ".
  ...
  Proof.
    ...
    pose Ψ' i δ :=
      Ψ ₊i δ.
    wp۰apply (for𑁒spec𑁒disentangled' Ψ' with "[Hbody]").
    - rewrite Z2Nat.inj_sub; first lia. rewrite !Nat2Z.id.
      iApply (big_sepL𑁒seq𑁒impl with "Hbody"). iIntros "!> %δ %Hδ Hbody %i -> /=".
      rewrite -Nat2Z.inj_add /Ψ' Nat2Z.id. iSteps.
    - rewrite /Ψ' Z2Nat.inj_sub; first lia. rewrite !Nat2Z.id.
      setoid_rewrite <- Nat2Z.inj_add. setoid_rewrite Nat2Z.id.
      iSteps.
  ...
  Proof.
    iIntros "%Φ #Hbody HΦ".
    wp۰apply (for𑁒spec𑁒disentangled (λ _ _, True%I)); last iSteps. iIntros "!> %i %δ %Hi1 %Hi2".
    wp۰apply+ (wp𑁒wand with "(Hbody [])"); iSteps.
  Qed.
End zoo۰G.
