...
Section zoo۰G.
  ...
  #[local] Lemma forｰspecｰstronger beg i δ Ψ _end body :
    i = (beg + δ)%Z →
    {{{
      ▷ Ψ i δ ∗
      □ (
  ...
  Proof.
    ...
    - rewrite decide_False; first lia.
      wp۰apply (wpｰwand with "(Hbody [//] [%] HΨ)") as "%res (-> & HΨ)"; first lia.
      iMod (lc_fupd_elim_later with "H£ HΨ") as "HΨ".
      wp۰apply+ ("IH" with "[] [] HΨ [HΦ]"); [iSteps.. |].
      assert ((i + 1) `max` _end = i `max` _end)%Z as -> by lia. rewrite -Nat.add_succ_comm //.
  Qed.
  Lemma forｰspecｰstrong Ψ beg _end body :
    {{{
      ▷ Ψ beg 0 ∗
      □ (
        ∀ i δ,
  ...
  Proof.
    ...
    wp۰apply (forｰspecｰstronger with "[$HΨ $Hbody]"); first lia.
    iSteps.
  Qed.
  Lemma forｰspec Ψ beg _end body :
    (beg ≤ _end)%Z →
    {{{
      ▷ Ψ beg 0 ∗
      □ (
  ...
  Proof.
    iIntros "% %Φ H HΦ".
    wp۰apply (forｰspecｰstrong Ψ with "H").
    rewrite Z.max_r //.
  Qed.
  Lemma forｰspecｰstrong' Ψ beg _end body :
    {{{
  ...
  Proof.
    ...
    wp۰apply (forｰspecｰstrong Ψ' with "[HΨ Hbody]"); last iSteps.
  ...
  Proof.
    ...
    rewrite Z.max_r //.
  Qed.
  Lemma forｰspecｰdisentangled Ψ beg _end body :
    {{{
      □ (
        ∀ i δ,
        ⌜i = (beg + δ)%Z⌝ -∗
  ...
  Proof.
    ...
    wp۰apply (forｰspecｰstrong Ψ'); last iSteps. iSplit; first iSteps. iIntros "!> %i %δ %Hi1 %Hi2 HΨ'".
    wp۰apply (wpｰwand with "(Hbody [//] [//])") as "%res (-> & HΨ)".
    iStep.
  ...
  Proof.
    ...
    wp۰apply (forｰspecｰstrong' Ψ' with "[Hbody]"); last iSteps. iSplit; first iSteps.
    iApply (big_sepLｰseqｰimpl with "Hbody"). iIntros "!> %δ %Hδ Hbody %i -> HΨ' /=".
    wp۰apply (wpｰwand with "(Hbody [//])") as "%res (-> & HΨ)".
    iStep.
    rewrite /Ψ' seq_S big_sepL_snoc. iSteps.
  Qed.

  Lemma forｰspecｰnatｰstrong Ψ beg _end body :
    {{{
      ▷ Ψ beg 0 ∗
      □ (
        ∀ i δ,
  ...
  Qed.
  Lemma forｰspecｰnat Ψ beg _end body :
    beg ≤ _end →
    {{{
      ▷ Ψ beg 0 ∗
      □ (
  ...
  Proof.
    iIntros "% %Φ H HΦ".
    wp۰apply (forｰspecｰnatｰstrong Ψ with "H").
    rewrite Nat.max_r //.
  Qed.
  Lemma forｰspecｰnatｰstrong' Ψ beg _end body :
    {{{
  ...
  Proof.
    ...
    pose Ψ' i δ :=
      Ψ ₊i δ.
    wp۰apply (forｰspecｰstrong' Ψ' with "[HΨ Hbody]").
  ...
  Proof.
    ...
    rewrite Nat.max_r //.
  Qed.
  Lemma forｰspecｰdisentangledｰnat Ψ beg _end body :
    {{{
      □ (
        ∀ i δ,
        ⌜i = (beg + δ)%nat⌝ -∗
  ...
  Proof.
    ...
    pose Ψ' i δ :=
      Ψ ₊i δ.
    wp۰apply (forｰspecｰdisentangled Ψ').
    - iIntros "!> %i %δ -> %Hδ".
  ...
  Proof.
    ...
    pose Ψ' i δ :=
      Ψ ₊i δ.
    wp۰apply (forｰspecｰdisentangled' Ψ' with "[Hbody]").
    - rewrite Z2Nat.inj_sub; first lia. rewrite !Nat2Z.id.
      iApply (big_sepLｰseqｰimpl with "Hbody"). iIntros "!> %δ %Hδ Hbody %i -> /=".
      rewrite -Nat2Z.inj_add /Ψ' Nat2Z.id. iSteps.
    - rewrite /Ψ' Z2Nat.inj_sub; first lia. rewrite !Nat2Z.id.
      setoid_rewrite <- Nat2Z.inj_add. setoid_rewrite Nat2Z.id.
      iSteps.
  ...
  Lemma forｰtype τ `{!iType (iProp Σ) τ} x beg _end body :
    {{{
  ...
  Proof.
    iIntros "%Φ #Hbody HΦ".
    wp۰apply (forｰspecｰdisentangled (λ _ _, True%I)); last iSteps. iIntros "!> %i %δ %Hi1 %Hi2".
    wp۰apply+ (wpｰwand with "(Hbody [])"); iSteps.
  Qed.
End zoo۰G.
