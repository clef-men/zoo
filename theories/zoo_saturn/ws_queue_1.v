...
Class WsQueue1G Σ `{zoo_G : !ZooG Σ} := {
  #[local] ws_queue_1_G_prophets_G :: WiseProphetsG Σ prophet ;
  #[local] ws_queue_1_G_model_G :: AuthTwinsG Σ (leibnizO (list val)) suffix ;
  #[local] ws_queue_1_G_owner_G :: TwinsG Σ (leibnizO (stability * nat * val * nat)) ;
  #[local] ws_queue_1_G_front_G :: AuthNatMaxG Σ ;
  #[local] ws_queue_1_G_history_G :: MonoListG Σ val ;
  #[local] ws_queue_1_G_winner_G :: TwinsG Σ (natO * leibnizO (option val) * ▶ ∙) ;
  #[local] ws_queue_1_G_datas_G :: MonoGmultisetG Σ val ;
}.
...
#[local] Definition min_capacity :=
  val_to_nat ws_queue_1_min_capacity.
...
: core.
#[local] Lemma ws_queue_1_min_capacity :
  ws_queue_1_min_capacity = #min_capacity.
Proof.
  done.
Qed.
Opaque ws_queue_1__code.ws_queue_1_min_capacity.
Opaque min_capacity.
...
Section ws_queue_1_G.
  ...
  #[local] Definition owner₁' γ_owner γ_model stable back data cap ws : iProp Σ :=
    twins_twin1 (twins_G := ws_queue_1_G_owner_G) γ_owner (DfracOwn 1) (stable, back, data, cap) ∗
  ...
  #[local] Definition owner₂' γ_owner stable back data cap :=
    twins_twin2 (twins_G := ws_queue_1_G_owner_G) γ_owner (stable, back, data, cap).
  #[local] Definition owner₂ γ :=
  ...
  Definition ws_queue_1_inv t ι : iProp Σ :=
    ∃ l γ,
    ⌜t = #l⌝ ∗
    ⌜ι = γ.(metadata_inv)⌝ ∗
    meta l nroot γ ∗
  ...
  Definition ws_queue_1_owner t ws : iProp Σ :=
  ...
  #[global] Instance ws_queue_1_model_timeless t vs :
    Timeless (ws_queue_1_model t vs).
  Proof.
    apply _.
  Qed.
  #[global] Instance ws_queue_1_owner_timeless t ws :
    Timeless (ws_queue_1_owner t ws).
  Proof.
    apply _.
  Qed.
  ...
  Proof.
    iMod (auth_twins_alloc _ (auth_twins_G := ws_queue_1_G_model_G)) as "(%γ_model & Hmodel_auth & Hmodel₁ & Hmodel₂)".
    iMod (twins_alloc' (twins_G := ws_queue_1_G_owner_G)) as "(%γ_owner & Howner₁ & Howner₂)".
    iFrameSteps.
  Qed.
  ...
  Lemma ws_queue_1_owner_exclusive t ws1 ws2 :
    ws_queue_1_owner t ws1 -∗
    ws_queue_1_owner t ws2 -∗
    False.
  Proof.
  ...
  Qed.
  Lemma ws_queue_1_owner_model t ws vs :
    ws_queue_1_owner t ws -∗
  ...
  Proof.
    ...
    rewrite ws_queue_1_min_capacity.
  ...
  Proof.
    ...
    assert (
      ∃ priv4,
      list.rotate ((front3 - front) `mod` cap2) ((priv ++ vs3) ++ priv2) = vs3 ++ priv4 ∧
    ...
    { destruct_decide (front3 = front + cap2) as -> | ?.
      ...
      - rewrite Nat.mod_small; first lia.
        rewrite /list.rotate drop_app -assoc drop_app_length'; first lia.
        eexists. split; [done | simpl_length; lia].
    }
    ...
    - iDestruct "Hwinner" as "(:winner_pending_1)".
      ...
      { destruct_decide (cap1 = 1) as Heq | ?.
        ...
        - rewrite Nat.mod_1_l; first lia.
          rewrite list.rotate_S; first lia.
          rewrite list.rotate_0 assoc //.
      }
  ...
  Lemma ws_queue_1_size_spec t ι ws :
    <<<
      ws_queue_1_inv t ι ∗
      ws_queue_1_owner t ws
    | ∀∀ vs,
  ...
  Proof.
    ...
    iModIntro. clear- Hcap Hus Hvs1 Hback Hsuffix.
  ...
  Lemma ws_queue_1_is_empty_spec t ι ws :
    <<<
      ws_queue_1_inv t ι ∗
      ws_queue_1_owner t ws
    | ∀∀ vs,
  ...
  Proof.
    ...
    wp_rec.
    wp_apply (ws_queue_1_size_spec with "[$]").
  ...
  Lemma ws_queue_1_push_spec t ι ws v :
    <<<
      ws_queue_1_inv t ι ∗
      ws_queue_1_owner t ws
    | ∀∀ vs,
  ...
  Inductive pop_state :=
    ...
    | PopEmptyishLoser
    | PopSuperempty.
  #[local] Lemma ws_queue_1_pop_0_spec {l γ} (state : pop_state) stable back (back_ : Z) data cap ws us id :
    back_ = back →
    {{{
      inv' l γ ∗
      owner' γ stable back data cap ws back us ∗
  ...
  Proof.
    ...
    - iDestruct "H" as "(%front & -> & #Hfront_lb & ->)".
      ...
      iSteps.
  Qed.
  Lemma ws_queue_1_pop_spec t ι ws :
    <<<
      ws_queue_1_inv t ι ∗
      ws_queue_1_owner t ws
    | ∀∀ vs,
  ...
  Proof.
    ...
    { iDestruct "Hstate" as "(:inv_state_empty =1 lazy=)".
      ...
      wp_smart_apply (ws_queue_1_pop_0_spec PopSuperempty _ (back - 1) with "[$Howner₁ $Hdata_cslice₂]"); [lia.. | iFrameSteps |].
      iSteps.
    }
    ...
    - assert (length vs1 = 0) as ->%nil_length_inv.
      ...
      + rewrite Hprophss1.
        ...
        wp_smart_apply (ws_queue_1_pop_0_spec (PopEmptyishWinner v) _ front1 with "[$Howner₁ $Hdata_cslice₂ $Hwinner_steal]"); [lia.. | iFrameSteps |].
        iSteps.
      ...
      + iDestruct "Hwinner" as "[(:winner) | Hwinner]".
        ...
        wp_smart_apply (ws_queue_1_pop_0_spec PopEmptyishLoser _ front1 with "[$Howner₁ $Hdata_cslice₂]"); [lia.. | iFrameSteps |].
        iSteps.
    ...
    - iMod (owner_update Stable (back - 1) data cap with "Howner₁ Howner₂") as "(Howner₁ & Howner₂)".
      ...
      wp_smart_apply (ws_queue_1_pop_0_spec (PopNonempty v) _ (back - 1) with "[$Howner₁ $Hdata_cslice₂]"); [lia.. | iFrameSteps |].
      iSteps.
  Qed.
End ws_queue_1_G.
