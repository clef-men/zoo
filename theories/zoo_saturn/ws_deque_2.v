...
Section ws_deque_2_G.
  ...
  Definition ws_deque_2_owner t ws : iProp Σ :=
    ∃ slots,
    ws_deque_1_owner t (#@{location} <$> slots) ∗
  ...
  #[global] Instance ws_deque_2_model_timeless t vs :
    Timeless (ws_deque_2_model t vs).
  Proof.
    apply _.
  Qed.
  #[global] Instance ws_deque_2_owner_timeless t ws :
    Timeless (ws_deque_2_owner t ws).
  Proof.
    apply _.
  Qed.
  ...
  Proof.
    ...
    iApply (ws_deque_1_model_exclusive with "Hmodel_1 Hmodel_2").
  Qed.
  ...
  Proof.
    ...
    iApply (ws_deque_1_owner_exclusive with "Howner_1 Howner_2").
  Qed.
  Lemma ws_deque_2_owner_model t ws vs :
    ws_deque_2_owner t ws -∗
    ws_deque_2_model t vs -∗
    ⌜vs `suffix_of` ws⌝.
  Proof.
    iIntros "(:owner =1) (:model)".
    iDestruct (ws_deque_1_owner_model with "Howner_1 Hmodel") as %(slots & ->)%suffix_fmap; last congruence.
  ...
  Proof.
    ...
    awp_apply (ws_deque_1_size_spec with "[$]") without "Hslots_ws".
  ...
  Proof.
    ...
    awp_apply (ws_deque_1_is_empty_spec with "[$]") without "Hslots_ws".
    iApply (aacc_aupd_commit with "HΦ"); first done. iIntros "%vs (:model)".
    iAaccIntro with "Hmodel"; first iSteps.
    iSteps --silent / as (Hsuffix%suffix_fmap) / as (_) "HΦ Howner".
    { iApply (big_sepL2_ref_pointsto_suffix with "Hslots_vs Hslots_ws"); first done. }
