...
#[local] Instance hintｰbig_sepMｰempty {PROP : bi} `{Countable K} {V} (Φ : K → V → PROP) :
  HINT ε₁ ✱ [- ;
    emp
  ] ⊫ [id];
    [∗ map] k ↦ v ∈ ∅, Φ k v
...
Module base.
  Section svar_G.
    ...
    Proof.
      ...
      wp۰block t as "Hmeta" "(Ht_value & Ht_gen & Ht_snapshot & Ht_proph & _)".
...
Section svar_G.
  ...
  Proof.
    ...
    { iApply (aaccｰaupdｰcommit with "HΦ"). 1: done. iIntros "%v' (:model =1)". simplify.
