...
#[local] Instance emptiness𑁒inhabited : Inhabited emptiness :=
  populate Empty.
#[local] Instance emptiness𑁒eq_dec : EqDecision emptiness :=
  ltac:(solve_decision).
...
#[local] Instance status𑁒eq_dec : EqDecision status :=
  ltac:(solve_decision).
...
#[local] Instance state۰le𑁒reflexive :
  Reflexive state۰le.
Proof.
  done.
Qed.
#[local] Instance state۰le𑁒transitive :
  Transitive state۰le.
Proof.
...
#[local] Lemma step𑁒mono state1 state2 :
  step state1 state2 →
...
Proof.
  ...
  - simpl. lia.
Qed.
#[local] Lemma steps𑁒mono state1 state2 :
  steps state1 state2 →
  state۰le state1 state2.
Proof.
  intros Hsteps.
...
#[global] Instance subG𑁒mpmc_queue_2۰Σ Σ `{zoo۰G : !ZooG Σ} :
...
#[local] Lemma suffix۰to_val𑁒generative i1 vs1 i2 vs2 :
...
#[local] Instance suffix۰to_val𑁒inj2 :
  Inj2 (=) (=) (=) suffix۰to_val.
Proof.
...
#[local] Instance suffix۰to_val𑁒inj2' :
  Inj2 (=) (=) (≈) suffix۰to_val.
Proof.
  intros ?* Hsimilar%suffix۰to_val𑁒generative.
...
#[local] Lemma prefix۰to_val𑁒generative i1 back1 vs1 i2 back2 vs2 :
...
#[local] Lemma prefix۰to_val𑁒inj i1 back1 vs1 i2 back2 vs2 :
  prefix۰to_val i1 back1 vs1 = prefix۰to_val i2 back2 vs2 →
    (vs1 ≠ [] → i1 = i2) ∧
    back1 = back2 ∧
    vs1 = vs2.
...
#[local] Lemma prefix۰to_val𑁒inj' i1 back1 vs1 i2 back2 vs2 :
  prefix۰to_val i1 back1 vs1 ≈ prefix۰to_val i2 back2 vs2 →
    ...
    vs1 = vs2.
Proof.
  intros Hsimilar%prefix۰to_val𑁒generative.
  apply prefix۰to_val𑁒inj. done.
Qed.
...
Section mpmc_queue_2۰G.
  ...
  #[local] Instance state۰auth𑁒timeless γ backs i status :
    Timeless (state۰auth γ backs i status).
  Proof.
    apply _.
  Qed.
  #[local] Instance state۰at𑁒timeless γ back i_back :
    Timeless (state۰at γ back i_back).
  Proof.
    apply _.
  Qed.
  #[global] Instance mpmc_queue_2۰model𑁒timeless t vs :
    Timeless (mpmc_queue_2۰model t vs).
  Proof.
    apply _.
  Qed.

  #[local] Instance state۰at𑁒persistent γ back i_back :
    Persistent (state۰at γ back i_back).
  Proof.
    apply _.
  Qed.
  #[global] Instance mpmc_queue_2۰inv𑁒persistent t ι :
  ...
  Proof.
    set state :=
      ...
      |}.
    iMod (auth_mono𑁒alloc _ (auth_mono۰G := mpmc_queue_2۰G۰state۰G) state) as "(%γ_state & $)".
    iSteps.
  Qed.
  #[local] Lemma state۰auth𑁒wf γ backs i status :
    state۰auth γ backs i status ⊢
    ⌜state۰wf backs i⌝.
  Proof.
    iSteps.
  Qed.
  #[local] Lemma state۰lb𑁒get γ backs i status :
    state۰auth γ backs i status ⊢
    state۰lb γ backs i status.
  Proof.
    iIntros "(:state۰auth)".
    iApply (auth_mono۰lb𑁒get with "Hauth").
  Qed.
  #[local] Lemma state۰at𑁒get {γ backs i status} back i_back :
    backs !! back = Some i_back →
    state۰auth γ backs i status ⊢
    state۰at γ back i_back.
  Proof.
    iIntros "%Hbacks_lookup (:state۰auth)".
    iDestruct (state۰lb𑁒get with "[$Hauth //]") as "#Hlb".
    iSteps.
  Qed.
  #[local] Lemma state۰lb𑁒valid γ backs1 i1 status1 backs2 i2 status2 :
  ...
  Proof.
    ...
    iDestruct (auth_mono۰lb𑁒valid with "Hauth Hlb") as %(? & ?)%steps𑁒mono. done.
  Qed.
  #[local] Lemma state۰lb𑁒valid𑁒Unstable γ backs1 i1 status1 backs2 i2 back2 move2 :
    state۰auth γ backs1 i1 status1 -∗
  ...
  Proof.
    iIntros "(:state۰auth) Hlb".
    iDestruct (auth_mono۰lb𑁒valid with "Hauth Hlb") as %[| (state & Hstep & (? & ?)%steps𑁒mono)]%rtc_inv.
    - naive_solver.
    - invert Hstep.
      ...
      iDestruct (state۰at𑁒get with "[$Hauth //]") as "Hstate_at"; first done.
      iRight. iSteps.
  Qed.
  #[local] Lemma state۰lb𑁒lookup {γ backs1 i1 status1 backs2 i2 status2} back i_back  :
  ...
  Proof.
    ...
    iDestruct (state۰lb𑁒valid with "Hauth Hlb") as %(? & _).
    iPureIntro. eapply lookup_weaken; done.
  Qed.
  #[local] Lemma state۰seen𑁒valid γ backs i status back i_prev back_prev move :
    state۰auth γ backs i status -∗
  ...
  Proof.
    ...
    iDestruct (state۰lb𑁒lookup  with "Hstate_auth Hstate_lb") as %Hbacks_lookup; first done.
    iDestruct (state۰lb𑁒valid𑁒Unstable with "Hstate_auth Hstate_lb") as "[(<- & -> & ->) | $]".
    all: iSteps.
  Qed.
  #[local] Lemma state۰at𑁒valid γ backs i status back i_back :
    state۰auth γ backs i status -∗
    state۰at γ back i_back -∗
      ⌜backs !! back = Some i_back⌝ ∗
      ⌜i_back ≤ i⌝.
  Proof.
    ...
    iDestruct (state۰lb𑁒lookup with "Hstate_auth Hstate_lb_1") as %Hbacks_lookup; first done.
    iDestruct (state۰lb𑁒valid with "Hstate_auth Hstate_lb_1") as "(_ & %Hi)".
    iSteps.
  Qed.
  (* #[local] Lemma state𑁒seen𑁒at𑁒agree γ backs i status back i_prev back_prev1 move back_prev2 : *)
  (*   state۰auth γ backs i status -∗ *)
  ...
  (*   iDestruct (state۰at𑁒valid with "Hstate_auth Hstate_at") as "#(%Hbacks_lookup_2 & _)". *)
  (*   iPureIntro. eapply state۰wf_inj; done. *)
  (* Qed. *)
  #[local] Lemma state۰lb𑁒stabilized γ backs1 i1 status1 backs2 i2 back2 move2 :
    ( status1 ≠ Unstable back2 move2
    ∨ i2 + length move2 ≤ i1 ∧
      0 < length move2
    ) →
  ...
  Proof.
    iIntros "% Hauth Hlb".
    iDestruct (state۰lb𑁒valid𑁒Unstable with "Hauth Hlb") as "[(-> & -> & ->) | ($ & _ & $)]".
    exfalso. naive_solver lia.
  Qed.
  #[local] Lemma state۰lb𑁒unstabilized γ backs1 i1 status1 backs2 i2 back2 move2 :
    i1 < i2 + length move2 →
    state۰auth γ backs1 i1 status1 -∗
    state۰lb γ backs2 i2 (Unstable back2 move2) -∗
      ⌜backs1 = backs2⌝ ∗
  ...
  Proof.
    iIntros "% Hauth Hlb".
    iDestruct (state۰lb𑁒valid𑁒Unstable with "Hauth Hlb") as "[(-> & -> & ->) | (_ & % & _)]"; first iSteps.
    exfalso. lia.
  Qed.
  #[local] Lemma state𑁒stabilize γ backs i back move :
    backs !! back = None →
  ...
  Proof.
    ...
    iMod (auth_mono𑁒update' with "Hauth") as "Hauth"; first eauto. simpl.
    ...
    iDestruct (state۰lb𑁒get with "[$Hauth //]") as "#Hstate_lb".
    iDestruct (state۰at𑁒get back with "[$Hauth //]") as "#Hat".
    { apply lookup_insert_eq. }
    iFrame "#∗". iSteps.
  Qed.
  #[local] Lemma state𑁒empty γ backs i :
    state۰auth γ backs i (Stable Nonempty) ⊢ |==>
    state۰auth γ backs i (Stable Empty).
  Proof.
    iIntros "(:state۰auth)".
    iMod (auth_mono𑁒update' with "Hauth") as "$"; first auto.
    iSteps.
  Qed.
  #[local] Lemma state𑁒destabilize {γ backs i} back move :
    state۰auth γ backs i (Stable Empty) ⊢ |==>
    state۰auth γ backs i (Unstable back move).
  Proof.
    iIntros "(:state۰auth)".
    iMod (auth_mono𑁒update' with "Hauth") as "$"; first eauto.
    iSteps.
  Qed.
  ...
  #[local] Lemma inv۰status𑁒weaken γ backs i status vs_front i_back back vs_back vs :
  ...
  Qed.
  #[local] Lemma inv۰status𑁒Stable strong γ backs i status vs_front i_back back vs_back vs :
  ...
  #[local] Lemma inv۰inner𑁒strengthen l γ :
    inv۰inner false l γ ⊢
    inv۰inner true l γ.
  Proof.
    iIntros "(:inv۰inner)".
    ...
    - iDestruct "Hstatus" as "(:inv۰status۰unstable)".
      ...
      { rewrite -eq_None_ne_Some.
        ...
        iApply (pointsto𑁒exclusive with "Hback_move Hback_move_").
      }

      iAssert (back_prev ↦ₕ Header §Back 2)%I as "#Hback_prev_header".
      { iDestruct (state۰at𑁒valid with "Hstate_auth Hstate_at_back_prev") as %(Hbacks_lookup_prev & _).
  ...
  Proof.
    ...
    { iDestruct (state۰at𑁒valid with "Hstate_auth Hstate_at") as %(Hbacks_lookup & _).
  ...
  Lemma mpmc_queue_2۰model𑁒exclusive t vs1 vs2 :
  ...
  Proof.
    ...
    wp۰apply (mpmc_queue_2٠rev₀𑁒spec i [v] with "Hback_header"); first auto.
  ...
  Proof.
    ...
    iMod (pointsto𑁒persist with "Hback_index") as "#Hback_index".
    wp۰block l as "Hmeta" "(Hl_front & Hl_back & _)".

    iMod model𑁒alloc as "(%γ_model & Hmodel₁ & Hmodel₂)".
    iMod (state𑁒alloc back) as "(%γ_state & Hstate_auth)".
    ...
    iDestruct (state۰lb𑁒get γ with "Hstate_auth") as "#Hstate_lb".
    iMod (state𑁒stabilize γ with "Hstate_auth") as "(Hstate_auth & _) /="; first done.
    iMod (state𑁒empty γ with "Hstate_auth") as "Hstate_auth".
    iDestruct (state۰at𑁒get (γ := γ) back 0 with "Hstate_auth") as "#Hstate_at".
    { apply lookup_insert_eq. }
  ...
  Proof.
    ...
    { destruct i_front as [i_state |]; last iSteps.
      iApply (front۰lb𑁒valid with "Hfront_auth Hfront_lb").
    }
    ...
    { destruct i_back as [i_back |]; last iSteps.
      ...
      iDestruct (state۰at𑁒valid with "Hstate_auth Hstate_at") as %(_ & ?).
      iSteps.
    }
  ...
  Proof.
    ...
    wp۰apply (front𑁒spec_strong None None with "[$Hinv //]").
    iSteps.
  Qed.
  ...
  Proof.
    ...
    iInv "Hinv" as "(:inv۰inner =1 >)".
    iDestruct (state۰lb𑁒valid𑁒Unstable with "Hstate_auth Hstate_lb") as "#[(-> & -> & ->) | (%Hbacks1_lookup & _)]".
  ...
  Proof.
    ...
    wp۰rec.
    wp۰apply (front𑁒spec with "Hinv") as (i_front1 vs_front1) "#Hfront_lb_1".

    wp۰apply+ (prophet_typed₁𑁒wp𑁒proph prophet_bool₁ with "[//]") as (pid proph) "Hproph".
    wp۰pures.
    ...
    - iDestruct (front۰lb𑁒valid with "Hfront_auth Hfront_lb_1") as %Hi_front2.
      ...
      { destruct status2.
        - iDestruct "Hstatus" as "(:inv۰status۰stable =2)".
          iDestruct (state۰at𑁒valid with "Hstate_auth Hstate_at_2") as %(Hbacks2_lookup & _).
      ...
      wp۰apply+ (front𑁒spec_strong (Some i_front2) None with "[$Hinv $Hfront_lb_2]") as (i_front3 vs_front3) "(_ & %Hi_front3 & _)".
      wp۰equal as _ | (-> & ->)%(inj2 _).
      all: wp۰apply+ (prophet_typed₁𑁒wp𑁒resolve with "Hproph"); [done.. |].
      all: iStep 12.
      wp۰apply (mpmc_queue_2٠suffix_index𑁒spec with "[//]") as "_".
      wp۰apply (mpmc_queue_2٠prefix_index𑁒spec with "[$]") as "_".
      wp۰pures.
    ...
    - iSplitR "Hproph HΦ". { iFrameSteps. }
      ...
      wp۰apply+ (front𑁒spec with "Hinv") as (i_front3 vs_front3) "_".
      wp۰equal.
      all: wp۰apply+ (prophet_typed₁𑁒wp𑁒resolve with "Hproph"); [done.. |].
      all: iSteps.
  Qed.
  ...
  Proof.
    ...
    awp۰apply (mpmc_queue_2٠size𑁒spec with "Hinv").
  ...
  Proof.
    ...
    iMod (inv'𑁒state۰at with "Hinv Hstate_at") as "(:back۰model₁)".

    wp۰rec. wp۰match.

    iInv "Hinv" as "(:inv۰inner =1 >)".
    iDestruct (state۰at𑁒valid with "Hstate_auth Hstate_at") as %(Hbacks1_lookup & _).
  ...
  Proof.
    ...
    iInv "Hinv" as "(:inv۰inner =1 >)".
    wp۰load.
    iDestruct (state۰lb𑁒get with "Hstate_auth") as "#Hstate_lb_1".
    ...
    - rewrite Nat.add_0_r in Hfront1. subst i_front1.
      ...
      + iDestruct (state۰lb𑁒stabilized with "Hstate_auth Hstate_lb") as "#(_ & #Hstate_at)"; first lia.
        ...
        wp۰apply+ (mpmc_queue_2٠finish𑁒spec with "[$] HΦ").

      + iDestruct (state۰lb𑁒unstabilized with "Hstate_auth Hstate_lb") as %(-> & -> & ->); first lia.
        ...
        * iSplitR "HΦ".
          { rewrite inv۰status𑁒weaken. iFrameSteps. }
          iSteps.

        * rewrite Nat.add_0_r in Hfront2. injection Hfront2 as <-.
          iDestruct (state۰lb𑁒unstabilized with "Hstate_auth Hstate_lb") as %(-> & _ & ->); first lia.
          iDestruct "Hstatus" as "(:inv۰status۰unstable =2 strong= lazy=)".

          iMod (state𑁒stabilize with "Hstate_auth") as "(Hstate_auth & _ & #Hstate_at)"; first done.
    ...
    - iAssert ⌜status1 ≠ Unstable back move⌝%I as %Hstabilized.
      ...
      iDestruct (state۰lb𑁒stabilized with "Hstate_auth Hstate_lb") as "#(_ & #Hstate_at)"; first auto.
  ...
  #[local] Lemma mpmc_queue_2٠push𑁒spec𑁒aux l γ v :
    ⊢ (
      ∀ back i ws (j : Z),
      <<<
        ⌜j = ⁺(i + length ws)⌝ ∗
  ...
  Proof.
    ...
    { iClear "IHpush_aux".
      ...
      wp۰cas as _ | (_ & -> & ->)%prefix۰to_val𑁒inj'.

      - iSplitR "HΦ".
        { rewrite inv۰status𑁒weaken. iFrameSteps. }
        iSteps.

      - iDestruct (inv۰status𑁒Stable with "Hstatus") as "(%empty1 & -> & (:inv۰status۰stable =1))"; first auto.

        iAssert ⌜i1 = i⌝%I as %->.
        { iDestruct (state۰at𑁒valid with "Hstate_auth Hstate_at_1") as %(Hbacks1_lookup & _). simplify.
          iSteps.
        }
      ...
      - iAssert (
        ...
        { destruct status1 as [empty1 | back1_ move1].
          - iDestruct "Hstatus" as "(:inv۰status۰stable =1)".
            iDestruct (state۰at𑁒valid with "Hstate_auth Hstate_at_1") as %(Hbacks_lookup & _).
          ...
          - iDestruct "Hstatus" as "(:inv۰status۰unstable =1 lazy=)".
            iDestruct "Hback1" as "(:back۰model₂ =1)".
            iDestruct (state۰lb𑁒get with "Hstate_auth") as "$".
            iSteps.
        }
        ...
        { iInv "Hinv" as "(:inv۰inner =2 >)".
          iDestruct (state۰lb𑁒valid𑁒Unstable with "Hstate_auth Hstate_lb_1") as "#[(-> & -> & ->) | (%Hbacks2_lookup & _ & #Hstate_at)]".
          - iDestruct "Hstatus" as "(:inv۰status۰unstable =2 >)".
            iDestruct "Hback2" as "(:back۰model₂ =2)".
            wp۰load.
            iDestruct (state۰at𑁒valid with "Hstate_auth Hstate_at_back2_prev") as %(Hbacks2_lookup & _).
          ...
          - iDestruct (big_sepM_lookup_acc with "Hbacks") as "((:back۰model₃ =1 !=) & Hbacks)"; first done.
            wp۰load.
            iDestruct (pointsto𑁒agree with "Hback1_index Hback1_index_") as %[= ->%(inj _)].
        ...
        + destruct move as [| w move]; first naive_solver lia.
          ...
          wp۰apply+ (mpmc_queue_2٠help𑁒spec with "[$]"); first done.
          iSteps.

      - iDestruct (inv۰status𑁒Stable with "Hstatus") as "(%empty1 & -> & (:inv۰status۰stable =1))"; first naive_solver lia.
  ...
  Proof.
    ...
    awp۰apply (mpmc_queue_2٠push𑁒spec𑁒aux with "Hinv").
    iApply (aacc𑁒aupd𑁒commit with "HΦ"); first done. iIntros "%vs (:model)". injection Heq as <-.
    iDestruct (meta𑁒agree with "Hmeta Hmeta_") as %<-. iClear "Hmeta_".
    iAaccIntro with "Hmodel₁"; iSteps.
  Qed.

  #[local] Lemma mpmc_queue_2٠pop𑁒spec𑁒aux l γ :
    ⊢ (
      ∀ i_front vs_front,
      <<<
        inv' l γ ∗
  ...
  Proof.
    ...
    { iIntros "%i_front %vs_front %Φ (#Hinv & #Hfront_lb) HΦ".
      ...
      - wp۰bind (_.{back})%E.
        iInv "Hinv" as "(:inv۰inner =1 >)".
        wp۰load.
        iDestruct (front۰lb𑁒valid with "Hfront_auth Hfront_lb") as %Hi_front1.
        iDestruct (front۰lb𑁒get with "Hfront_auth") as "#Hfront_lb_1".
        ...
        + iAssert (
          ...
          { destruct status1 as [empty1 | back1_ move1].
            - iDestruct "Hstatus" as "(:inv۰status۰stable =1)".
              iDestruct (state۰at𑁒valid with "Hstate_auth Hstate_at_1") as %(Hbacks1_lookup & _).
              iDestruct (big_sepM_lookup with "Hbacks") as "(:back۰model₃ =1)"; first done.
              iDestruct "H𝑚𝑜𝑣𝑒1" as "(:move۰model₂ =1)".
              iFrame "#".
            - iDestruct "Hstatus" as "(:inv۰status۰unstable =1 lazy=)".
        ...
        + iDestruct (inv۰status𑁒Stable with "Hstatus") as "(%empty1 & -> & (:inv۰status۰stable =1))"; first naive_solver lia.
          ...
          * assert (length vs_front1 = 0) as ->%nil_length_inv by lia.
            ...
            -- ospecialize* Hcas; first done. subst i_back2.
               iDestruct (inv۰status𑁒Stable with "Hstatus") as "(%empty2 & -> & (:inv۰status۰stable =2))"; first naive_solver lia.
          ...
          * wp۰block back as "#Hback_header" "_" "(Hback_index & Hback_move & _) /=".
            wp۰match.
            wp۰apply (front𑁒spec_strong (Some i_front1) (Some i1) with "[$Hinv $Hfront_lb_1 $Hstate_at_1]") as (i_front3 vs_front3) "(#Hfront_lb_3 & %Hi_front3 & (%i3 & %Hi3 & %Hfront3))".
            ...
            -- ospecialize* Hcas; first done. subst i_back4.
               iDestruct (inv۰status𑁒Stable with "Hstatus") as "(%empty4 & -> & (:inv۰status۰stable =4))"; first naive_solver lia.
               iDestruct (front۰lb𑁒valid with "Hfront_auth Hfront_lb") as %Hi_front4.
               replace i_front4 with ˖i4 in * by lia.
               destruct empty4; last lia. subst vs_front4.

               iMod (state𑁒destabilize with "Hstate_auth") as "Hstate_auth".
               iDestruct (state۰lb𑁒get with "Hstate_auth") as "#Hstate_lb_4".
               iSplitR "HΦ".
               { iFrameSteps. iExists _, _, []. iSteps. }
               iIntros "!> {%- Hvs_back1}".
               ...
               { iSteps.
                 iMod (inv'𑁒state۰at with "Hinv Hstate_at_1") as "(:back۰model₁ =1)".
                 iSteps.
               }
      ...
      - wp۰bind (𝗰𝗮𝘀 _ _ _)%E.
        ...
        + iDestruct (inv۰status𑁒Stable with "Hstatus") as "(%empty1 & -> & (:inv۰status۰stable =1))"; first naive_solver lia.
          ...
          { destruct (nil_or_length_pos vs_front) as [-> | Hvs_front].
            1: iMod (state𑁒empty with "Hstate_auth") as "Hstate_auth".
    ...
    { iClear "IHpop_1 IHpop_2".
      ...
      wp۰rec.
      wp۰apply+ (mpmc_queue_2٠rev𑁒spec with "[$]") as "_"; first lia.
      ...
      - rewrite Nat.add_0_r in Hfront1. subst i_front.
        iDestruct (state۰lb𑁒valid with "Hstate_auth Hstate_lb") as %(_ & ?).
        replace i1 with i by lia.

        iDestruct (state۰lb𑁒unstabilized with "Hstate_auth Hstate_lb") as %(-> & _ & ->). lia.
        ...
        wp۰apply+ (mpmc_queue_2٠finish𑁒spec with "[$]").
        iSteps.
    }
    ...
    { iClear "IHpop_2 IHpop_3 IHpop".
      ...
      wp۰rec.
      wp۰apply+ (front𑁒spec with "Hinv") as (i_front1 vs_front1) "#Hfront_lb".
  ...
  Proof.
    ...
    awp۰apply (mpmc_queue_2٠pop𑁒spec𑁒aux with "Hinv").
    iApply (aacc𑁒aupd𑁒commit with "HΦ"); first done. iIntros "%vs (:model)". injection Heq as <-.
    iDestruct (meta𑁒agree with "Hmeta Hmeta_") as %<-. iClear "Hmeta_".
