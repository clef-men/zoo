...
Class MpmcQueue2G Σ `{zoo_G : !ZooG Σ} := {
  #[local] mpmc_queue_3_G_model_G :: TwinsG Σ (leibnizO (list val)) ;
  #[local] mpmc_queue_3_G_lstate_G :: AuthMonoG (A := leibnizO lstate) Σ lstep ;
}.

Definition mpmc_queue_3_Σ := #[
  twins_Σ (leibnizO (list val)) ;
  auth_mono_Σ (A := leibnizO lstate) lstep
].
#[global] Instance subG_mpmc_queue_3_Σ Σ `{zoo_G : !ZooG Σ} :
  subG mpmc_queue_3_Σ Σ →
  MpmcQueue2G Σ.
Proof.
  solve_inG.
Qed.
...
Section mpmc_queue_3_G.
  Context `{mpmc_queue_3_G : MpmcQueue2G Σ}.
  ...
  Definition mpmc_queue_3_inv t ι : iProp Σ :=
    ∃ l γ,
    ⌜t = #l⌝ ∗
    meta l nroot γ ∗
    inv ι (inv_inner l γ).
  ...
  Definition mpmc_queue_3_model t vs : iProp Σ :=
    ∃ l γ,
    ⌜t = #l⌝ ∗
    meta l nroot γ ∗
    model₁ γ vs.
  ...
  #[global] Instance mpmc_queue_3_model_timeless t vs :
    Timeless (mpmc_queue_3_model t vs).
  Proof.
    apply _.
  Qed.
  #[global] Instance mpmc_queue_3_inv_persistent t ι :
    Persistent (mpmc_queue_3_inv t ι).
  Proof.
    apply _.
  Qed.
  ...
  Lemma mpmc_queue_3_create_spec ι :
    {{{
      True
    }}}
      mpmc_queue_3_create ()
    {{{ t,
      RET t;
      mpmc_queue_3_inv t ι ∗
      mpmc_queue_3_model t []
    }}}.
  Proof.
  Admitted.

  Lemma mpmc_queue_3_size_spec t ι :
    <<<
      mpmc_queue_3_inv t ι
    | ∀∀ vs,
      mpmc_queue_3_model t vs
    >>>
      mpmc_queue_3_size t @ ↑ι
    <<<
      mpmc_queue_3_model t vs
    | sz,
      RET #sz;
  ...
  Lemma mpmc_queue_3_is_empty_spec t ι :
    <<<
      mpmc_queue_3_inv t ι
    | ∀∀ vs,
      mpmc_queue_3_model t vs
    >>>
      mpmc_queue_3_is_empty t @ ↑ι
    <<<
      mpmc_queue_3_model t vs
    | b,
      RET #b;
  ...
  #[local] Lemma mpmc_queue_3_finish_spec l γ ι backs i lstatus back :
    back ∈ dom backs →
    {{{
      meta l nroot γ ∗
      inv ι (inv_inner l γ) ∗
  ...
  Lemma mpmc_queue_3_push_back_spec t ι v :
    <<<
      mpmc_queue_3_inv t ι
    | ∀∀ vs,
      mpmc_queue_3_model t vs
    >>>
      mpmc_queue_3_push_back t v @ ↑ι
    <<<
      mpmc_queue_3_model t (vs ++ [v])
    | RET ();
      True
    >>>.
  Proof.
  ...
  Lemma mpmc_queue_3_push_front_spec t ι v :
    <<<
      mpmc_queue_3_inv t ι
    | ∀∀ vs,
      mpmc_queue_3_model t vs
    >>>
      mpmc_queue_3_push_front t v @ ↑ι
    <<<
      mpmc_queue_3_model t (v :: vs)
    | RET ();
      True
    >>>.
  Proof.
  ...
  Lemma mpmc_queue_3_pop_spec t ι :
    <<<
      mpmc_queue_3_inv t ι
    | ∀∀ vs,
      mpmc_queue_3_model t vs
    >>>
      mpmc_queue_3_pop t @ ↑ι
    <<<
      mpmc_queue_3_model t (tail vs)
    | RET head vs;
      True
    >>>.
  Proof.
...
#[global] Opaque mpmc_queue_3_create.
#[global] Opaque mpmc_queue_3_size.
#[global] Opaque mpmc_queue_3_is_empty.
#[global] Opaque mpmc_queue_3_push_back.
#[global] Opaque mpmc_queue_3_push_front.
#[global] Opaque mpmc_queue_3_pop.

#[global] Opaque mpmc_queue_3_inv.
#[global] Opaque mpmc_queue_3_model.
