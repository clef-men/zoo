...
Class InfWsQueue1G Σ `{zoo_G : !ZooG Σ} := {
  ...
  #[local] inf_ws_queue_1_G_prophets_G :: WiseProphetsG Σ prophet ;
  #[local] inf_ws_queue_1_G_model_G :: AuthTwinsG Σ (leibnizO (list val)) suffix ;
  #[local] inf_ws_queue_1_G_owner_G :: TwinsG Σ (leibnizO (stability * nat * (nat → val))) ;
  #[local] inf_ws_queue_1_G_front_G :: AuthNatMaxG Σ ;
  #[local] inf_ws_queue_1_G_history_G :: MonoListG Σ val ;
  #[local] inf_ws_queue_1_G_winner_G :: TwinsG Σ (natO * ▶ ∙) ;
}.
...
Section inf_ws_queue_1_G.
  ...
  #[local] Definition owner₁' γ_owner γ_model stable back priv ws : iProp Σ :=
    twins_twin1 (twins_G := inf_ws_queue_1_G_owner_G) γ_owner (DfracOwn 1) (stable, back, priv) ∗
  ...
  Proof.
    apply _.
  Qed.
  #[global] Instance inf_ws_queue_1_owner_timeless t ws :
    Timeless (inf_ws_queue_1_owner t ws).
  Proof.
    apply _.
  Qed.
  ...
  Lemma inf_ws_queue_1_size_spec t ι ws :
    <<<
      inf_ws_queue_1_inv t ι ∗
      inf_ws_queue_1_owner t ws
    | ∀∀ vs,
  ...
  Lemma inf_ws_queue_1_is_empty_spec t ι ws :
    <<<
      inf_ws_queue_1_inv t ι ∗
      inf_ws_queue_1_owner t ws
    | ∀∀ vs,
  ...
  Proof.
    ...
    wp_rec.
    wp_apply (inf_ws_queue_1_size_spec with "[$]").
  ...
  Lemma inf_ws_queue_1_push_spec t ι ws v :
    <<<
      inf_ws_queue_1_inv t ι ∗
      inf_ws_queue_1_owner t ws
    | ∀∀ vs,
  ...
  #[local] Lemma inf_ws_queue_1_pop_0_spec {l γ} (state : pop_state) stable back (back_ : Z) priv ws id :
    ...
    {{{
      ...
      inf_ws_queue_1_pop_0 #l #id #back_
    {{{ o back priv,
      RET o : val;
      owner₁ γ Stable back priv ws ∗
      match state with
