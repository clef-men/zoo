...
Section waiters_G.
  ...
  Proof.
    ...
    wp_apply (array_unsafe_init_spec_disentangled (λ _ 𝑤𝑎𝑖𝑡𝑒𝑟,
      waiter_inv 𝑤𝑎𝑖𝑡𝑒𝑟
    )%I) as (waiters 𝑤𝑎𝑖𝑡𝑒𝑟𝑠) "(%H𝑤𝑎𝑖𝑡𝑒𝑟𝑠 & Hwaiters & H𝑤𝑎𝑖𝑡𝑒𝑟𝑠)". 1: done.
    { iIntros "!> %i %Hi".
      wp_apply (waiter_create_spec with "[//]").
      iSteps.
    }
  ...
  Lemma waiters_notify_spec t sz :
    {{{
      waiters_inv t sz
    }}}
      waiters_notify t
  ...
  Proof.
    ...
    wp_apply+ (waiter_notify_spec with "H𝑤𝑎𝑖𝑡𝑒𝑟") as ([]) "_". 1: iSteps.
    wp_apply+ ("HLöb" with "HΦ").
  Qed.

  Lemma waiters_notify_all_spec t sz :
    {{{
      waiters_inv t sz
    }}}
  ...
  Proof.
    ...
    wp_apply+ (waiter_notify_spec with "H𝑤𝑎𝑖𝑡𝑒𝑟") as (res) "_".
    wp_apply+ ("HLöb" with "HΦ").
  Qed.

  Lemma waiters_prepare_wait_spec t (sz : nat) i :
    (0 ≤ i < sz)%Z →
    {{{
      waiters_inv t sz
    }}}
  ...
  Proof.
    ...
    wp_rec.
    wp_apply+ (array_unsafe_get_spec with "Hwaiters") as "_". 1-3: done || lia.
    wp_apply+ (waiter_prepare_wait_spec with "H𝑤𝑎𝑖𝑡𝑒𝑟") as "_".

    awp_apply+ (mpmc_queue_1_push_spec with "Hqueue_inv") without "HΦ".
    iInv "Hinv" as "(:inv_inner)".
    iAaccIntro with "Hqueue_model". 1: iSteps. iIntros "Hqueue_model !>".
    iSplitL. { iFrameSteps. }
    iSteps.
  ...
  Lemma waiters_cancel_wait_spec t (sz : nat) i :
    (0 ≤ i < sz)%Z →
    {{{
      waiters_inv t sz
    }}}
  ...
  Proof.
    ...
    wp_rec.
    wp_apply+ (array_unsafe_get_spec with "Hwaiters") as "_". 1-3: done || lia.
    wp_apply+ (waiter_cancel_wait_spec with "H𝑤𝑎𝑖𝑡𝑒𝑟 HΦ").
  Qed.

  Lemma waiters_commit_wait_spec t (sz : nat) i :
    (0 ≤ i < sz)%Z →
    {{{
      waiters_inv t sz
    }}}
  ...
  Proof.
    ...
    wp_rec.
    wp_apply+ (array_unsafe_get_spec with "Hwaiters") as "_". 1-3: done || lia.
    wp_apply+ (waiter_commit_wait_spec with "H𝑤𝑎𝑖𝑡𝑒𝑟 HΦ").
  Qed.
End waiters_G.
