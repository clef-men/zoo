...
Class McasG Σ `{zoo_G : !ZooG Σ} :=
  { #[local] mcas_G_model_G :: TwinsG Σ val_O
  ; #[local] mcas_G_helper_G :: SavedPropG Σ
  ; #[local] mcas_G_post_G :: SavedPredG Σ bool
  ; #[local] mcas_G_lstatus_G :: AuthMonoG (A := leibnizO lstatus) Σ lstep
  ; #[local] mcas_G_history_G :: MonoListG Σ location
  ; #[local] mcas_G_lock_G :: ExclG Σ unitO
  ; #[local] mcas_G_helpers_G :: ghost_mapG Σ gname nat
  ; #[local] mcas_G_winning_G :: ExclG Σ unitO
  ; #[local] mcas_G_owner_G :: ExclG Σ unitO
  }.
...
#[global] Instance subG_mcas_Σ Σ `{zoo_G : !ZooG Σ} :
  subG mcas_Σ Σ →
  McasG Σ.
Proof.
  solve_inG.
...
Section mcas_G.
  ...
  Definition mcas_loc_inv loc ι : iProp Σ :=
    ∃ γ,
    loc ↪ γ ∗
    loc_inv' ι (loc, γ).

  Definition mcas_loc_model loc v : iProp Σ :=
    ∃ γ,
    loc ↪ γ ∗
    model₁ γ v.
  ...
  Proof.
    apply _.
  Qed.
  #[global] Instance mcas_loc_model_timeless loc ι :
    Timeless (mcas_loc_model loc ι).
  Proof.
    apply _.
  Qed.
  ...
  Proof.
    ...
    apply _.
  Qed.
  #[global] Instance mcas_loc_inv_persistent loc γ ι :
    Persistent (mcas_loc_inv loc ι).
  Proof.
    rewrite /mcas_loc_inv.
    apply _.
  Qed.
  ...
  Lemma mcas_loc_model_exclusive loc v1 v2 :
    mcas_loc_model loc v1 -∗
    mcas_loc_model loc v2 -∗
    False.
  Proof.
  ...
  #[local] Lemma mcas٠status_to_bool𑁒spec fstatus :
    {{{
      True
    }}}
      mcas٠status_to_bool (final_status_to_val fstatus)
    {{{
  ...
  #[local] Lemma mcas٠clear𑁒spec casn η ι b :
    b = metadata_success η →
    {{{
      casn_inv' ι casn η ∗
      lstatus_lb η Finished
    }}}
      mcas٠clear (metadata_cass_val η) #b
    {{{
      RET ();
      True
    }}}.
  ...
  #[local] Lemma mcas٠finish𑁒spec {gid casn η ι} fstatus :
    {{{
      casn ↪ η ∗
      casn_inv' ι casn η ∗
      ...
      mcas٠finish #gid #casn (final_status_to_val fstatus)
    {{{
  ...
  Proof.
    ...
    - iDestruct "Hlstatus" as "(:casn_inv_inner_running)".
      ...
      wp_apply+ (mcas٠status_to_bool𑁒spec with "[//]") as "_".
      wp_load. wp_pures.
      ...
      { destruct b; last iSteps.
        wp_apply+ (mcas٠clear𑁒spec with "[$Hcasn_inv' $Hlstatus_lb]"); first auto.
        iSteps.
      }
  ...
  Qed.
  #[local] Lemma mcas٠finish𑁒spec_loser {gid casn η ι} fstatus :
    gid ≠ metadata_winner η →
    {{{
      casn ↪ η ∗
      casn_inv' ι casn η ∗
  ...
  Proof.
    ...
    wp_apply (mcas٠finish𑁒spec with "[- HΦ] HΦ").
    iSteps.
  Qed.
  #[local] Lemma mcas٠finish𑁒spec_winner_before gid casn η ι Ψ :
    gid = metadata_winner η →
    {{{
      casn ↪ η ∗
      casn_inv' ι casn η ∗
  ...
  Proof.
    ...
    wp_apply (mcas٠finish𑁒spec FinalBefore with "[- HΦ] HΦ").
    iSteps.
  Qed.
  #[local] Lemma mcas٠finish𑁒spec_after {gid casn η ι} i :
    metadata_size η ≤ i →
    {{{
      casn ↪ η ∗
      casn_inv' ι casn η ∗
  ...
  Proof.
    ...
    wp_apply (mcas٠finish𑁒spec FinalAfter with "[- HΦ] HΦ").
    destruct_decide (gid = metadata_winner η); iSteps.
  Qed.
  #[local] Lemma mcas٠finish𑁒spec_finished gid casn η ι :
    {{{
      casn ↪ η ∗
      casn_inv' ι casn η ∗
      lstatus_lb η Finished
  ...
  #[local] Lemma mcas٠determine_as_eval_determine𑁒spec ι :
    ⊢ (
      ∀ casn η 𝑐𝑎𝑠𝑠 i,
      {{{
      ...
      {{{
        ...
        mcas٠lock #casn #descr.(descriptor_loc) #descr1.(descriptor_state) #descr.(descriptor_state) 𝑟𝑒𝑡𝑟𝑦 𝑐𝑜𝑛𝑡𝑖𝑛𝑢𝑒
      {{{
        RET #(metadata_success η);
        lstatus_lb η Finished
      }}}
      ...
      {{{
        ...
        mcas٠eval #descr.(descriptor_state)
      {{{
  ...
  Proof.
    ...
    { iIntros "%casn %η %𝑐𝑎𝑠𝑠 %i !> %Φ (-> & #Hcasn_meta & #Hcasn_inv' & #Hlstatus_lb) HΦ".
      ...
      - apply lookup_lt_Some in Hdescrs_lookup as Hi.
        ...
        + iMod (descriptor_state_inj with "Hcasn_inv' Hcasn1_inv'") as %?; [done.. |].
          ...
          * iInv "Hcasn_inv" as "(:casn_inv_inner)".
            ...
            all: iStep 12.
            wp_apply (mcas٠finish𑁒spec_winner_before with "[- HΦ] HΦ"); first done.
            iSteps.
          ...
          * iSplitR "Hgid Hlproph HΦ". { iFrameSteps. }
            ...
            -- iDestruct "Hbefore" as "[-> | #Hlstatus_lb_finished]".
               ...
               ++ destruct Hok as [(Hgid & _ & _) | Hbefore%not_and_l].
                  ...
                  ** wp_apply (mcas٠finish𑁒spec_loser FinalBefore with "[$Hcasn_meta $Hcasn_inv' $Hgid] HΦ"); first done.

                  ** exfalso. naive_solver.

               ++ wp_apply (mcas٠finish𑁒spec_finished with "[$Hcasn_meta $Hcasn_inv' $Hlstatus_lb_finished] HΦ").
      ...
      - rewrite drop_lookup_None //.
        { rewrite list_lookup_fmap Hdescrs_lookup //. }
        wp_apply+ (mcas٠finish𑁒spec_after with "[$Hcasn_meta $Hcasn_inv' $Hgid $Hlstatus_lb] HΦ").
  ...
  Qed.
  #[local] Lemma mcas٠determine_as𑁒spec casn η ι 𝑐𝑎𝑠𝑠 i :
    𝑐𝑎𝑠𝑠 = list_to_val (drop i (metadata_cass η)) →
    {{{
      casn ↪ η ∗
      casn_inv' ι casn η ∗
  ...
  Proof.
    iDestruct mcas٠determine_as_eval_determine𑁒spec as "(H & _)".
    iIntros (->) "%Φ (#Hcasn_meta & #Hcasn_inv' & #Hlstatus_lb) HΦ".
    wp_apply ("H" with "[$Hcasn_meta $Hcasn_inv' $Hlstatus_lb //] HΦ").
  Qed.
  #[local] Lemma mcas٠eval𑁒spec {casn η ι} i descr :
    η.(metadata_descrs) !! i = Some descr →
    {{{
      casn ↪ η ∗
      casn_inv' ι casn η
  ...
  Proof.
    iDestruct mcas٠determine_as_eval_determine𑁒spec as "(_ & _ & H & _)".
    iIntros "%Hdescrs_lookup %Φ (#Hcasn_meta & #Hcasn_inv') HΦ".
    wp_apply ("H" with "[$Hcasn_meta $Hcasn_inv' //] HΦ").
  Qed.
  #[local] Lemma mcas٠determine𑁒spec casn η ι :
    {{{
      casn ↪ η ∗
      casn_inv' ι casn η
    }}}
  ...
  Proof.
    iDestruct mcas٠determine_as_eval_determine𑁒spec as "(_ & _ & H)".
    iIntros "%Φ (#Hcasn_meta & #Hcasn_inv') HΦ".
    wp_apply ("H" with "[$Hcasn_meta $Hcasn_inv' //] HΦ").
  Qed.

  Lemma mcas٠make𑁒spec ι v :
    {{{
      True
    }}}
      mcas٠make v
  ...
  Proof.
    ...
    wp_apply (mcas٠eval𑁒spec with "[$Hcasn_meta $Hcasn_inv']") as "(#Hlstatus_lb & H£2)"; first done.
  ...
  Lemma mcas٠mcas𑁒spec {ι 𝑠𝑝𝑒𝑐} locs befores afters :
    ...
    <<<
      [∗ list] loc ∈ locs, mcas_loc_inv loc ι
    | ∀∀ vs,
      [∗ list] loc; v ∈ locs; vs, mcas_loc_model loc v
    >>>
      mcas٠mcas 𝑠𝑝𝑒𝑐 @ ↑ι
    <<<
      ∃∃ b,
      if b then
        ⌜vs ≈ befores⌝ ∗
        [∗ list] loc; v ∈ locs; afters, mcas_loc_model loc v
      else
        ∃ i before v,
  ...
  Proof.
    ...
    iApply wp_fupd.
    wp_apply+ (mcas٠determine_as𑁒spec with "[$Hcasn_meta $Hcasn_inv' $Hlstatus_lb]") as "#Hlstatus_lb_finished"; first done.
...
#[global] Opaque mcas_loc_inv.
#[global] Opaque mcas_loc_model.
