...
Fixpoint erase_ectx_item k : ectx :=
  match k with
  | CtxAppL v2 =>
      [CtxAppL (erase_val v2)]
  | CtxAppR e1 =>
      [CtxAppR (erase_expr e1)]
  | CtxUnop op =>
      [CtxUnop op]
  | CtxBinopL op v2 =>
      [CtxBinopL op (erase_val v2)]
  | CtxBinopR op e1 =>
      [CtxBinopR op (erase_expr e1)]
  | CtxIf e1 e2 =>
      [CtxIf (erase_expr e1) (erase_expr e2)]
  | CtxPairL v2 =>
      [CtxPairL (erase_val v2)]
  | CtxPairR e1 =>
      [CtxPairR (erase_expr e1)]
  | CtxFst =>
      [CtxFst]
  | CtxSnd =>
      [CtxSnd]
  ...
  | CtxInjr =>
      [CtxInjr]
  | CtxCase e1 e2 =>
      [CtxCase (erase_expr e1) (erase_expr e2)]
  | CtxAllocL v2 =>
      [CtxAllocL (erase_val v2)]
  | CtxAllocR e1 =>
      [CtxAllocR (erase_expr e1)]
  | CtxLoad =>
      [CtxLoad]
  | CtxStoreL v2 =>
      [CtxStoreL (erase_val v2)]
  | CtxStoreR e1 =>
      [CtxStoreR (erase_expr e1)]
  | CtxCasL v1 v2 =>
      [CtxCasL (erase_val v1) (erase_val v2)]
  | CtxCasM e0 v2 =>
      [CtxCasM (erase_expr e0) (erase_val v2)]
  | CtxCasR e0 e1 =>
      [CtxCasR (erase_expr e0) (erase_expr e1)]
  | CtxFaaL v2 =>
      [CtxFaaL (erase_val v2)]
  | CtxFaaR e1 =>
      [CtxFaaR (erase_expr e1)]
  | CtxResolveL ctx v1 v2 =>
      erase_ectx_item ctx ++
      [CtxPairL (erase_val v1); CtxPairL (erase_val v2); CtxFst; CtxFst]
  | CtxResolveM e0 v2 =>
      [CtxPairR (erase_expr e0); CtxPairL (erase_val v2); CtxFst; CtxFst]
  | CtxResolveR e0 e1 =>
      [CtxPairR (erase_expr e0, erase_expr e1); CtxFst; CtxFst]
  end.

Definition erase_ectx K : ectx :=
  mbind erase_ectx_item K.

Definition erase_tp es :=
  erase_expr <$> es.
...
Definition erase_cfg ρ : cfg zebra :=
...
Lemma erase_ectx_app K K' :
  erase_ectx (K ++ K') = erase_ectx K ++ erase_ectx K'.
Proof.
...
Theorem erasure e σ ϕ :
  adequate NotStuck e σ ϕ →
  adequate NotStuck (erase_expr e) (erase_state σ)
           (λ v σ, ∃ v' σ', erase_val v' = v ∧ erase_state σ' = σ ∧ ϕ v' σ').
Proof.
