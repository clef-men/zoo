...
Section ectx_language_mixin.
  ...
  Record EctxLanguageMixin := {
    ...
    mixin_of_to_val e v :
      to_val e = Some v →
      of_val v = e ;
    mixin_val_base_stuck tid e1 σ1 κ e2 σ2 es :
    ...
    mixin_step_by_val tid K' K_redex e1' e1_redex σ1 κ e2 σ2 es :
      fill K' e1' = fill K_redex e1_redex →
      to_val e1' = None →
      base_step tid e1_redex σ1 κ e2 σ2 es →
        ∃ K'',
        K_redex = comp_ectx K' K'' ;

    mixin_base_ctx_step_val tid K e σ1 κ e2 σ2 es :
...
Section ectx_language.
  ...
  Qed.
  Lemma base_reducible_not_val tid e σ :
  ...
  Lemma step_by_val tid K' K_redex e1' e1_redex σ1 κ e2 σ2 es :
    fill K' e1' = fill K_redex e1_redex →
    to_val e1' = None →
    base_step tid e1_redex σ1 κ e2 σ2 es →
      ∃ K'',
  ...
  Lemma base_ctx_step_val tid K e σ1 κ e2 σ2 es :
    base_step tid (fill K e) σ1 κ e2 σ2 es →
      is_Some (to_val e) ∨
      K = empty_ectx.
  Proof.
  ...
  Lemma base_step_fill_prim_step tid K e1 σ1 κ e2 σ2 es :
    base_step tid e1 σ1 κ e2 σ2 es →
  ...
  Qed.
  Lemma not_base_reducible tid e σ :
    ¬ base_reducible tid e σ ↔
    base_irreducible tid e σ.
  Proof.
  ...
  Lemma base_redex_unique tid K e K' e' σ :
    fill K e = fill K' e' →
    base_reducible tid e σ →
    base_reducible tid e' σ →
  ...
  Proof.
    ...
    edestruct (step_by_val tid K' K e' e) as [K'' HK]; [eauto using base_step_not_val.. |].
    subst K. move: Heq. rewrite -fill_comp. intros <-%(inj (fill _)).
    odestruct base_ctx_step_val as [[]%not_eq_None_Some | HK''].
    { done. }
  ...
  Lemma base_step_not_stuck tid e σ κ e' σ' es :
    base_step tid e σ κ e' σ' es →
    not_stuck tid e σ.
  Proof.
  ...
  Lemma fill_prim_step tid K e1 σ1 κ e2 σ2 es :
    prim_step tid e1 σ1 κ e2 σ2 es →
    prim_step tid (fill K e1) σ1 κ (fill K e2) σ2 es.
  Proof.
  ...
  Qed.
  Lemma fill_reducible tid K e σ :
    reducible tid e σ →
  ...
  Qed.
  Lemma fill_reducible_no_obs tid K e σ :
    reducible_no_obs tid e σ →
    reducible_no_obs tid (fill K e) σ.
  Proof.
  ...
  Qed.
  Lemma base_reducible_prim_fill_reducible tid e K σ :
    base_reducible tid e σ →
    reducible tid (fill K e) σ.
  Proof.
    intro.
  ...
  Qed.
  Lemma base_reducible_no_obs_reducible tid e σ :
    base_reducible_no_obs tid e σ →
    reducible_no_obs tid e σ.
  Proof.
    intros (e' & σ' & es & ?).
    ...
    apply base_step_prim_step. done.
  Qed.
  Lemma irreducible_base_irreducible tid e σ :
    irreducible tid e σ →
    base_irreducible tid e σ.
  Proof.
  ...
  Qed.
  Lemma base_reducible_no_obs_fill_reducible tid e K σ :
    base_reducible_no_obs tid e σ →
    reducible_no_obs tid (fill K e) σ.
  Proof.
    intro.
  ...
  Lemma reducible_base_reducible tid e σ :
    reducible tid e σ →
  ...
  Qed.
  Lemma base_irreducible_irreducible tid e σ :
    base_irreducible tid e σ →
    sub_redexes_are_values e →
    irreducible tid e σ.
  Proof.
  ...
  Qed.
  Lemma base_stuck_stuck tid e σ :
    base_stuck tid e σ →
    sub_redexes_are_values e →
    stuck tid e σ.
  Proof.
  ...
  Proof.
    intros.
    edestruct (base_reducible_fill_prim_step tid empty_ectx) as (? & ? & ?).
  ...
  Proof.
    ...
    - intros ? ? ? ? ? ? ? [K' e1' e2' Heq1 Heq2 Hstep].
      ...
      all: rewrite ?Heq1 ?Heq2 ?fill_comp //.
...
#[global] Arguments base_step_fill_prim_step' {_ _ _ _ _ _ _ _}.
