...
#[global] Instance subGｰauth_frac۰Σ Σ A :
...
Section auth_frac۰G.
  ...
  Proof.
    solve_proper.
  Qed.
  #[global] Instance auth_frac۰fragｰproper γ q :
    Proper ((≡) ==> (≡)) (auth_frac۰frag γ q).
  Proof.
    solve_proper.
  Qed.

  #[global] Instance auth_frac۰authｰtimeless γ x :
    Discrete x →
    Timeless (auth_frac۰auth γ x).
  Proof.
    apply _.
  Qed.
  #[global] Instance auth_frac۰fragｰtimeless γ q y :
    Discrete y →
  ...
  Lemma auth_frac۰authｰvalid `{!CmraDiscrete A} γ x :
    auth_frac۰auth γ x ⊢
    ⌜✓ x⌝.
  Proof.
    iIntros "H●".
  ...
  Lemma auth_frac۰fragｰvalid `{!CmraDiscrete A} γ q y :
  ...
  Qed.
  Lemma auth_frac۰fragｰsplit {γ q y} q1 y1 q2 y2 :
    q = (q1 + q2)%Qp →
    y = y1 ⋅ y2 →
    auth_frac۰frag γ q y ⊢
  ...
  Proof.
    iIntros (-> ->) "($ & $)".
  Qed.
  Lemma auth_frac۰fragｰcombine γ q1 y1 q2 y2 :
    auth_frac۰frag γ q1 y1 -∗
    auth_frac۰frag γ q2 y2 -∗
  ...
  Proof.
    ...
    iCombine "H◯1 H◯2" as "$".
  Qed.
  Lemma auth_frac۰fragｰvalidｰ2 `{!CmraDiscrete A} γ q1 y1 q2 y2 :
    auth_frac۰frag γ q1 y1 -∗
    auth_frac۰frag γ q2 y2 -∗
      ⌜q1 ⋅ q2 ≤ 1⌝%Qp ∗
      ⌜✓ (y1 ⋅ y2)⌝.
  Proof.
    iIntros "H◯1 H◯2".
    iDestruct (auth_frac۰fragｰcombine with "H◯1 H◯2") as "H◯".
    iDestruct (auth_frac۰fragｰvalid with "H◯") as "$".
  Qed.
  Lemma auth_frac۰fragｰfracｰne `{!CmraDiscrete A} γ1 q1 y1 γ2 q2 y2 :
    ¬ (q1 + q2 ≤ 1)%Qp →
  ...
  Proof.
    iIntros "% H◯1 H◯2 ->".
    iDestruct (auth_frac۰fragｰvalidｰ2 with "H◯1 H◯2") as "(% & _)" => //.
  Qed.
  Lemma auth_frac۰fragｰne `{!CmraDiscrete A} γ1 y1 γ2 q2 y2 :
    auth_frac۰frag γ1 1 y1 -∗
    auth_frac۰frag γ2 q2 y2 -∗
    ⌜γ1 ≠ γ2⌝.
  Proof.
    iApply auth_frac۰fragｰfracｰne.
    { intros []%(exclusive_l 1%Qp). }
  Qed.
  Lemma auth_frac۰fragｰexclusive `{!CmraDiscrete A} γ y1 q2 y2 :
    auth_frac۰frag γ 1 y1 -∗
    auth_frac۰frag γ q2 y2 -∗
    False.
  Proof.
    iIntros "H◯1 H◯2".
    iDestruct (auth_frac۰fragｰne with "H◯1 H◯2") as %? => //.
  Qed.

  Lemma auth_fracｰauthｰfragｰagree `{!CmraDiscrete A} γ x y :
    auth_frac۰auth γ x -∗
  ...
  Qed.
  Lemma auth_fracｰauthｰfragｰagreeｰL `{!CmraDiscrete A, !LeibnizEquiv A} γ x y :
    auth_frac۰auth γ x -∗
    auth_frac۰frag γ 1 y -∗
    ⌜x = y⌝.
  Proof.
    iIntros "H● H◯".
    iDestruct (auth_fracｰauthｰfragｰagree with "H● H◯") as %?%leibniz_equiv => //.
  Qed.

  Lemma auth_fracｰauthｰfragｰincluded `{!CmraDiscrete A, !CmraTotal A} γ x q y :
    auth_frac۰auth γ x -∗
    auth_frac۰frag γ q y -∗
    ⌜y ≼ x⌝.
  Proof.
  ...
  Lemma auth_fracｰupdate {γ x q y} x' y' :
    (x, y) ~l~> (x', y') →
    auth_frac۰auth γ x -∗
    auth_frac۰frag γ q y ==∗
      auth_frac۰auth γ x' ∗
  ...
  Qed.
  Lemma auth_fracｰupdateｰ1 {γ x y} x' :
    ✓ x' →
    auth_frac۰auth γ x -∗
    auth_frac۰frag γ 1 y ==∗
      auth_frac۰auth γ x' ∗
...
Section auth_frac۰G.
  ...
  Lemma auth_frac۰fragｰdivide {γ q y} ys :
    y = foldr (⋅) ε ys →
    auth_frac۰frag γ q y ⊢
  ...
  Proof.
    ...
    - destruct_decide (length ys = 0) as ->%nil_length_inv | Hys.
      ...
      + assert (q = q / (1 + Qp۰of_nat (length ys)) + (q * Qp۰of_nat (length ys) / (1 + Qp۰of_nat (length ys))))%Qp as Heq.
        ...
        iDestruct (auth_frac۰fragｰsplit with "H◯") as "($ & H◯)". 1,2: done.
        iDestruct ("IH" with "[//] H◯") as "H◯s".
        iEval (rewrite Qp.div_div Qp.div_mul_cancel_r) in "H◯s".
        iFrame.
  Qed.
  Lemma auth_frac۰fragｰdivide' {γ q y} q' ys :
  ...
  Proof.
    iIntros (-> ->) "H◯".
    iDestruct (auth_frac۰fragｰdivide with "H◯") as "H◯s". 1: done.
    iEval (rewrite -{2}(Qp.mul_1_r (Qp۰of_nat _)) Qp.div_mul_cancel_l Qp.div_1) in "H◯s".
    iFrame.
  Qed.
  Lemma auth_frac۰fragｰgather γ q ys :
    0 < length ys →
    ([∗ list] y ∈ ys, auth_frac۰frag γ q y) ⊢
  ...
  Proof.
    ...
    - iDestruct "H◯s" as "(H◯ & H◯s)".
      ...
      + iEval (rewrite Qp۰of_natｰ1 Qp.mul_1_l right_id).
        iFrame.
      + iEval (rewrite Qp۰of_natｰS //).
        iEval (rewrite Qp.mul_add_distr_r Qp.mul_1_l).
        iApply (auth_frac۰fragｰcombine with "H◯").
