...
#[global] Instance subGｰghost_list۰Σ Σ A :
...
Section ghost_list۰G.
  ...
  Proof.
    apply _.
  Qed.
  #[global] Instance ghost_list۰atｰtimeless γ i dq x :
    Timeless (ghost_list۰at γ i dq x).
  Proof.
    apply _.
  Qed.

  #[global] Instance ghost_list۰atｰpersistent γ i x :
    Persistent (ghost_list۰at γ i DfracDiscarded x).
  Proof.
    apply _.
  Qed.

  #[global] Instance ghost_list۰atｰfractional γ i x :
    Fractional (λ q, ghost_list۰at γ i (DfracOwn q) x).
  Proof.
    apply _.
  Qed.
  #[global] Instance ghost_list۰atｰas_fractional γ i q x :
    AsFractional (ghost_list۰at γ i (DfracOwn q) x) (λ q, ghost_list۰at γ i (DfracOwn q) x) q.
  Proof.
    apply _.
  Qed.

  Lemma ghost_listｰalloc xs :
    ⊢ |==>
      ∃ γ,
  ...
  Proof.
    ...
    rewrite big_sepMｰmap_seqｰ0 //.
  Qed.

  Lemma ghost_list۰authｰexclusive γ xs1 xs2 :
  ...
  Lemma ghost_list۰atｰvalid γ i dq x :
    ghost_list۰at γ i dq x ⊢
    ⌜✓ dq⌝.
  Proof.
    iApply ghost_map_elem_valid.
  Qed.
  Lemma ghost_list۰atｰcombine γ i dq1 x1 dq2 x2 :
  ...
  Qed.
  Lemma ghost_list۰atｰvalidｰ2 γ i dq1 x1 dq2 x2 :
    ghost_list۰at γ i dq1 x1 -∗
    ghost_list۰at γ i dq2 x2 -∗
      ⌜✓ (dq1 ⋅ dq2)⌝ ∗
      ⌜x1 = x2⌝.
  Proof.
    iIntros "Hat1 Hat2".
    iDestruct (ghost_list۰atｰcombine with "Hat1 Hat2") as "(-> & Hat)".
    iDestruct (ghost_list۰atｰvalid with "Hat") as "$".
    iSteps.
  Qed.
  Lemma ghost_list۰atｰagree γ i dq1 x1 dq2 x2 :
    ghost_list۰at γ i dq1 x1 -∗
    ghost_list۰at γ i dq2 x2 -∗
    ⌜x1 = x2⌝.
  Proof.
    iIntros "Hat1 Hat2".
    iDestruct (ghost_list۰atｰvalidｰ2 with "Hat1 Hat2") as "(_ & $)".
  Qed.
  Lemma ghost_list۰atｰdfracｰne γ1 i1 dq1 x1 γ2 i2 dq2 x2 :
    ¬ ✓ (dq1 ⋅ dq2) →
  ...
  Proof.
    ...
    iDestruct (ghost_list۰atｰvalidｰ2 with "Hat1 Hat2") as "(% & _)". done.
  Qed.
  Lemma ghost_list۰atｰne γ1 i1 x1 γ2 i2 dq2 x2 :
    ghost_list۰at γ1 i1 (DfracOwn 1) x1 -∗
    ghost_list۰at γ2 i2 dq2 x2 -∗
    ⌜γ1 ≠ γ2 ∨ i1 ≠ i2⌝.
  Proof.
    iApply ghost_list۰atｰdfracｰne; [done.. | intros []%(exclusive_l _)].
  Qed.
  Lemma ghost_list۰atｰexclusive γ i x1 dq2 x2 :
  ...
  Lemma ghost_listｰlookup γ xs i dq x :
    ghost_list۰auth γ xs -∗
    ghost_list۰at γ i dq x -∗
    ⌜xs !! i = Some x⌝.
  Proof.
    ...
    iSteps.
  Qed.
  Lemma ghost_listｰauthｰats γ xs1 dq xs2 :
  ...
  Qed.
  Lemma ghost_listｰupdateｰat {γ xs i x} x' :
    ghost_list۰auth γ xs -∗
    ghost_list۰at γ i (DfracOwn 1) x ==∗
  ...
  Proof.
    iIntros "Hauth Hat".
    iDestruct (ghost_listｰlookup with "Hauth Hat") as %?%lookup_lt_Some.
