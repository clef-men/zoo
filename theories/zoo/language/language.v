...
Lemma base_reducible_no_obs_equal v1 v2 σ :
  base_reducible_no_obs (Equal (Val v1) (Val v2)) σ.
Proof.
  destruct (val_similar_or_nonsimilar v1 v2).
  all: repeat econstructor; done.
Qed.
Lemma base_reducible_equal v1 v2 σ :
  base_reducible (Equal (Val v1) (Val v2)) σ.
Proof.
  apply base_reducible_no_obs_base_reducible, base_reducible_no_obs_equal.
Qed.
Lemma reducible_equal v1 v2 σ :
  reducible (Equal (Val v1) (Val v2)) σ.
Proof.
  apply base_reducible_reducible, base_reducible_equal.
Qed.

Lemma base_reducible_no_obs_cas l fld v1 v2 v σ :
  σ.(state_heap) !! (l +ₗ fld) = Some v →
  base_reducible_no_obs (CAS (Val $ ValTuple [ValLoc l; ValInt fld]) (Val v1) (Val v2)) σ.
Proof.
  destruct (val_similar_or_nonsimilar v v1).
  all: repeat econstructor; done.
Qed.
Lemma base_reducible_cas l fld v1 v2 v σ :
  σ.(state_heap) !! (l +ₗ fld) = Some v →
  base_reducible (CAS (Val $ ValTuple [ValLoc l; ValInt fld]) (Val v1) (Val v2)) σ.
Proof.
  intros.
  eapply base_reducible_no_obs_base_reducible, base_reducible_no_obs_cas. done.
Qed.
Lemma reducible_cas l fld v1 v2 v σ :
  σ.(state_heap) !! (l +ₗ fld) = Some v →
  reducible (CAS (Val $ ValTuple [ValLoc l; ValInt fld]) (Val v1) (Val v2)) σ.
Proof.
  intros.
  eapply base_reducible_reducible, base_reducible_cas. done.
Qed.

Lemma reducible_resolve e σ pid v :
  Atomic e →
  reducible e σ →
  reducible (Resolve e (Val $ ValProph pid) (Val v)) σ.
Proof.
  intros A (κ & e' & σ' & es & H).
...
Qed.
Lemma prim_step_resolve_inv e v1 v2 σ1 κ e2 σ2 es :
  Atomic e →
  prim_step (Resolve e (Val v1) (Val v2)) σ1 κ e2 σ2 es →
  base_step (Resolve e (Val v1) (Val v2)) σ1 κ e2 σ2 es.
Proof.
  intros A [K e1' e2' Hfill -> Hstep]. simpl in *.
