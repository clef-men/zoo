...
Section ws_deques.
  ...
  #[using="ws_deques"]
  Definition ws_hub_std_push_foreign : val :=
    λ: "t" "v",
  ...
  #[local] Definition ws_hub_std_pop_foreign : val :=
    λ: "t",
  ...
  #[local] Definition ws_hub_std_try_steal_aux yield : val :=
    rec: "ws_hub_std_try_steal_aux" "t" "i" "max_round" :=
      if: "max_round" ≤ #0 then (
        ...
        match: ws_hub_std_pop_foreign "t" with
        | Some <> as "res" =>
            "res"
        | None =>
...
Class WsHub1G Σ `{zoo_G : !ZooG Σ} := {
  #[local] ws_hub_std_G_queue_G :: MpmcQueueG Σ ;
...
Section ws_hub_std_G.
  ...
  Proof.
    ...
    pose decode := λ '(γ_size, γ_deques, γ_foreign, γ_rounds, γ_waiters, γ_model), {|
      ...
      ws_hub_std_meta_foreign := γ_foreign ;
  ...
  #[local] Definition ws_hub_std_inv_inner l γ : iProp Σ :=
    ...
    mpmc_queue_model γ.(ws_hub_std_meta_foreign) vs_foreign ∗
  ...
  Definition ws_hub_std_inv t ι : iProp Σ :=
    ...
    l.[foreign] ↦□ γ.(ws_hub_std_meta_foreign) ∗
    ...
    mpmc_queue_inv γ.(ws_hub_std_meta_foreign) (ι.@"foreign") ∗
  ...
  Proof.
    ...
    pose γ := {|
      ...
      ws_hub_std_meta_foreign := foreign ;
  ...
  Lemma ws_hub_std_push_foreign_spec t ι v :
    <<<
      ws_hub_std_inv t ι
    | ∀∀ vs,
      ws_hub_std_model t vs
    >>>
      ws_hub_std_push_foreign ws_deques t v @ ↑ι
    <<<
      ws_hub_std_model t ({[+v+]} ⊎ vs)
    | RET (); True
    >>>.
  ...
  #[local] Lemma ws_hub_std_pop_foreign_spec t ι :
    <<<
      ws_hub_std_inv t ι
    | ∀∀ vs,
      ws_hub_std_model t vs
    >>>
      ws_hub_std_pop_foreign t @ ↑ι
    <<<
      ∃∃ o,
      match o with
      | None =>
  ...
  Proof.
    ...
    - awp_apply ws_hub_std_pop_foreign_spec without "Hdeques_owner Hround"; first iSteps.
...
#[global] Opaque ws_hub_std_push_foreign.
